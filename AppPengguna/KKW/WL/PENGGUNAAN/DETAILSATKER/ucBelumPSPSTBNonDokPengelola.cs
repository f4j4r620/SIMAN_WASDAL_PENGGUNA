...
{
    public partial class ucBelumPSPSTBNonDokPengelola : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA221.InputParameters input;
        SvcMonSatkerGunaA221.OutputParameters output;
        SvcMonSatkerGunaA221.execute_pttClient client;
        SvcMonSatkerGunaA221.WASDALSROW_MON_GUNA_SATKER_A221 rowData;

        public ucBelumPSPSTBNonDokPengelola()
        {
            InitializeComponent();
        }

        public ucBelumPSPSTBNonDokPengelola(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA221.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA221.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA221.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA221.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A221.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A221);
