...
{
    class UcRwyMutJlnJmbtn : UserControlDetail
    {
        ...
        private SvcRwyMutJlnJmbtnSelect.call_pttClient fetchData;
        private SvcRwyMutJlnJmbtnSelect.InputParameters parInp;
        private SvcRwyMutJlnJmbtnSelect.OutputParameters outData;
        public SvcRwyMutJlnJmbtnSelect.BPSIMANSROW_KJALJ_RWYT_MUTASI selectedData;



        public UcRwyMutJlnJmbtn(decimal? _ID_KJALJ, String Status)
            : base()
        {
            this.initTnhRMutasi();
            this.Status = Status;
        ...
        private void initTnhRMutasi()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcRwyMutJlnJmbtnSelect.BPSIMANSROW_KJALJ_RWYT_MUTASI);
            ...
            this.setKolom("Kuantitas", "KUANTITAS", "KUANTITAS", 4, true, 120, "string", false, "right");
        ...
        public void getInitTnhRMutasi(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcRwyMutJlnJmbtnSelect.InputParameters();
            parInp.P_COL = "";
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KJALJ = {0} {1}", this.ID_KJALJ, this.search);
            fetchData = new SvcRwyMutJlnJmbtnSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcRwyMutJlnJmbtnSelect.OutputParameters dataOut);

        public void showData(SvcRwyMutJlnJmbtnSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI.Count();
            ...
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date = Convert.ToString(serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 10);
              string date_ = Convert.ToString(serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 8);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 10);
              string date2_ = Convert.ToString(serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 8);
              if (date == "11/11/1000" || date_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_BUKU = null;
              }
              if (date2 == "11/11/1000" || date2_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i].TGL_DSR_MUTASI = null;
              }
                binder.Add(serviceOutPut.SF_ROW_KJALJ_RWYT_MUTASI[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcRwyMutJlnJmbtnSelect.BPSIMANSROW_KJALJ_RWYT_MUTASI)selectedView.GetRow(e.FocusedRowHandle);
