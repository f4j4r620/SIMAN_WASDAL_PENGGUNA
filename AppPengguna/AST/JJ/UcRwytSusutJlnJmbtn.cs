...
{
    class UcRwytSusutJlnJmbtn : UserControlDetail
    {
        private SvcJlnJmbtnSusutSelect.call_pttClient fetchData;
        private SvcJlnJmbtnSusutSelect.InputParameters parInp;
        private SvcJlnJmbtnSusutSelect.OutputParameters outData;
        public SvcJlnJmbtnSusutSelect.BPSIMANSROW_M_KJALJ_RWYT_SUSUT selectedData;
        ...
        public UcRwytSusutJlnJmbtn(decimal? _ID_KJALJ, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
            this.IdKjlnJmbtn = _ID_KJALJ;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcJlnJmbtnSusutSelect.BPSIMANSROW_M_KJALJ_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcJlnJmbtnSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KJALJ = {0} {1}", this.IdKjlnJmbtn, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcJlnJmbtnSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcJlnJmbtnSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcJlnJmbtnSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date = Convert.ToString(serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
              string date_ = Convert.ToString(serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 8);
              if (date == "11/11/1000" || date_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[i].TGL_SUSUT = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KJALJ_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcJlnJmbtnSusutSelect.BPSIMANSROW_M_KJALJ_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
