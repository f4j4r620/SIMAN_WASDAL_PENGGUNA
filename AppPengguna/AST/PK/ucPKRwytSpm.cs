...
{
    class ucPKRwytSpm : UserControlDetail
    {
        private frmSpm puSpm;
        private SvcPropertiKhususSpmSelect.call_pttClient fetchData;
        private SvcPropertiKhususSpmSelect.InputParameters parInp;
        private SvcPropertiKhususSpmSelect.OutputParameters outData;
        public SvcPropertiKhususSpmSelect.BPSIMANSROW_M_KPROK_RWYT_SPM selectedData;
        ...
        public ucPKRwytSpm(decimal? _ID_KPROK, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
            this.IdKspm = _ID_KPROK;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcPropertiKhususSpmSelect.BPSIMANSROW_M_KPROK_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcPropertiKhususSpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KPROK = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcPropertiKhususSpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcPropertiKhususSpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcPropertiKhususSpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_BUKU).Substring(0, 10);
                string date_ = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_BUKU).Substring(0, 8);
                string date2 = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_SP2D).Substring(0, 10);
                string date2_ = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_SP2D).Substring(0, 8);
                if (date == "11/11/1000" || date_ == "1/1/0001")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_BUKU = null;
                    
                }
                if (date2 == "11/11/1000" || date2_ == "1/1/0001")
                {
                    
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i].TGL_SP2D = null;
                }
                binder.Add(serviceOutPut.SF_ROW_M_KPROK_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcPropertiKhususSpmSelect.BPSIMANSROW_M_KPROK_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
