...
{
    class ucPKRwytSusut : UserControlDetail
    {
        private frmPenyusutan puSusut;
        private SvcPropertiKhususSusutSelect.call_pttClient fetchData;
        private SvcPropertiKhususSusutSelect.InputParameters parInp;
        private SvcPropertiKhususSusutSelect.OutputParameters outData;
        public SvcPropertiKhususSusutSelect.BPSIMANSROW_M_KPROK_RWYT_SUSUT selectedData;
        ...
        public ucPKRwytSusut(decimal? _ID_KPROK, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
            this.IdKpk = _ID_KPROK;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcPropertiKhususSusutSelect.BPSIMANSROW_M_KPROK_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcPropertiKhususSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KPROK = {0} {1}", this.IdKpk, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcPropertiKhususSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcPropertiKhususSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcPropertiKhususSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
                string date_ = Convert.ToString(serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 8);
                if (date == "11/11/1000" || date_ == "1/1/0001")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].TGL_SUSUT = null;
                }
                if (serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].NO_SPPA == "-")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].NO_SPPA = null;
                }
                if (serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].UR_TRN == "-")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].UR_TRN = null;
                }
                if (serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].UR_SUSUT == "-")
                {
                    serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i].UR_SUSUT = null;
                }
                binder.Add(serviceOutPut.SF_ROW_M_KPROK_RWYT_SUSUT[i]);
            }
        ...
        protected override void bbEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                puSusut = new frmPenyusutan(selectedData, "detail");
                puSusut.ShowDialog();
            }
        }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcPropertiKhususSusutSelect.BPSIMANSROW_M_KPROK_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
