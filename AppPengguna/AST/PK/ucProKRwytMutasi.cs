...
{
    class ucProKRwytMutasi : UserControlDetail
    {
        ...
        private SvcProkRwytMutasi.call_pttClient fetchData;
        private SvcProkRwytMutasi.InputParameters parInp;
        private SvcProkRwytMutasi.OutputParameters outData;
        public SvcProkRwytMutasi.BPSIMANSROW_KPROK_RWYT_MUTASI selectedData;



        public ucProKRwytMutasi(decimal? _ID_KPROK, String Status)
            : base()
        {
            this.initTnhRMutasi();
            this.Status = Status;
        ...
        private void initTnhRMutasi()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcProkRwytMutasi.BPSIMANSROW_KPROK_RWYT_MUTASI);
        ...
        public void getInitTnhRMutasi(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcProkRwytMutasi.InputParameters();
            parInp.P_COL = "";
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KPROK = {0} {1}", this.ID_KPROK, this.search);
            fetchData = new SvcProkRwytMutasi.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcProkRwytMutasi.OutputParameters dataOut);

        public void showData(SvcProkRwytMutasi.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI.Count();
            ...
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date = Convert.ToString(serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 10);
                string date_ = Convert.ToString(serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 8);
                string date2 = Convert.ToString(serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 10);
                string date2_ = Convert.ToString(serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 8);
                if (date == "11/11/1000" || date_ == "1/1/0001")
                {
                    serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_BUKU = null;
                    
                }
                if (date2 == "11/11/1000" || date2_ == "1/1/0001")
                {
                    
                    serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].TGL_DSR_MUTASI = null;
                }
                if (serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].SATUAN == "-")
                {
                    serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].SATUAN = null;
                }
                if (serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].NO_DSR_MUTASI == "-")
                {
                    serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i].NO_DSR_MUTASI = null;
                }
                binder.Add(serviceOutPut.SF_ROW_KPROK_RWYT_MUTASI[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcProkRwytMutasi.BPSIMANSROW_KPROK_RWYT_MUTASI)selectedView.GetRow(e.FocusedRowHandle);
