...
{
    class UcKDPRwySpm : UserControlDetail
    {
        private SvcKDPSpmSelect.call_pttClient fetchData;
        private SvcKDPSpmSelect.InputParameters parInp;
        private SvcKDPSpmSelect.OutputParameters outData;
        public SvcKDPSpmSelect.BPSIMANSROW_M_KDP_RWYT_SPM selectedData;
        ...
        public UcKDPRwySpm(decimal? _ID_KDP, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcKDPSpmSelect.BPSIMANSROW_M_KDP_RWYT_SPM);
            ...
            this.setKolom("Nilai KDP", "RPH_KDP", "RPH_KDP", 8, true);
            this.setKolom("Nilai SPM", "JML_SPM", "JML_SPM", 9, true);
            this.setKolom("Keterangan", "KETERANGAN", "KETERANGAN", 10, true);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcKDPSpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KDP = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcKDPSpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcKDPSpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcKDPSpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KDP_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KDP_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KDP_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KDP_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                binder.Add(serviceOutPut.SF_ROW_M_KDP_RWYT_SPM[i]);
            }
        ...
        private string get_where_clause(string nama_kolom, string opr, string parameter, string parameter2)
        {
            ...
            switch (nama_kolom)
            {
                ...
                case "Kode Transaksi":
                    ...
                    break;
                case "Nilai KDP":
                    where = "Upper(RPH_KDP) " + get_operator("String", opr, parameter, parameter2);
                    break;
                case "Nilai SPM":
                    where = "Upper(JML_SPM) " + get_operator("String", opr, parameter, parameter2);
                    break;
                case "Keterangan":
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcKDPSpmSelect.BPSIMANSROW_M_KDP_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
