...
{
    class UcPeggRmhNgr : UserControlDetail
    {


        private frmPengelola puPengelola;
        private String Status;
        public decimal? ID_KRMH_NEG_RWYT_PENGGUNA;
        private decimal? ID_KRMH_NEG;
        private string search;
        public frmProgres progresBar = null;

        private SvcRwyPggRmhNgrSelect.call_pttClient fetchData;
        private SvcRwyPggRmhNgrSelect.InputParameters parInp;
        private SvcRwyPggRmhNgrSelect.OutputParameters outData;
        public SvcRwyPggRmhNgrSelect.BPSIMANSROW_KRMH_NEG_RWYT_PENGGUNA selectedData;



        public UcPeggRmhNgr(decimal? _ID_KRMH_NEG, String Status)
            : base()
        {
            this.initRiwayatPengguna();
            this.Status = Status;
        ...
        private void initRiwayatPengguna()
        {

            this.binder = new BindingSource();
            this.binder.DataSource = typeof(AppPengguna.SvcRwyPggRmhNgrSelect.BPSIMANSROW_KRMH_NEG_RWYT_PENGGUNA);
        ...
        protected void view_dokumen(object sender, ItemClickEventArgs e)
        {
            try
            {
                ...
                parInp.P_ID = selectedData.ID_KRMH_NEG_RWYT_PENGGUNA;
                parInp.P_IDSpecified = true;
                parInp.P_ID_TABLE = "ID_KRMH_NEG_RWYT_PENGGUNA";
                parInp.P_TABLE = "M_KRMH_NEG_RWYT_PENGGUNA";
        ...
        public void getInitTnhRPgn(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcRwyPggRmhNgrSelect.InputParameters();
            parInp.P_COL = "";
            ...
            fetchData = new SvcRwyPggRmhNgrSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcRwyPggRmhNgrSelect.OutputParameters dataOut);
        private string StatusCrud = "";
        private decimal? NUM;
        public void showData(SvcRwyPggRmhNgrSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA.Count();

            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_KONTRAK).Substring(0, 10);
              string date_ = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_KONTRAK).Substring(0, 8);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_MULAI).Substring(0, 10);
              string date2_ = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_MULAI).Substring(0, 8);
              string date3 = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SELESAI).Substring(0, 10);
              string date3_ = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SELESAI).Substring(0, 8);
              string date4 = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SK).Substring(0, 10);
              string date4_ = Convert.ToString(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SK).Substring(0, 8);

              if (date == "11/11/1000" || date_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_KONTRAK = null;
              }
              if (date2 == "11/11/1000" || date2_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_MULAI = null;
              }
              if (date3 == "11/11/1000" || date3_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SELESAI = null;
              }
              if (date4 == "11/11/1000" || date4_ == "1/1/0001")
              {
                serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i].TGL_SK = null;
              }
                binder.Add(serviceOutPut.SF_ROW_KRMH_NEG_RWYT_PENGGUNA[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcRwyPggRmhNgrSelect.BPSIMANSROW_KRMH_NEG_RWYT_PENGGUNA)selectedView.GetRow(e.FocusedRowHandle);
        ...
        SvcRumahRpengelolaCrud.call_pttClient svcRiwayatPengelolaCrud;
        SvcRumahRpengelolaCrud.OutputParameters outRiwayatPengelolaCrud;

        private void saveForm(string text)
        {
          if (this.puPengelola.teTglSKPenetapan.Text != "" || this.puPengelola.teSKPenetapan.Text != "")
          {
            ...
            try
            {
              ...
              SvcRumahRpengelolaCrud.InputParameters parInp = new SvcRumahRpengelolaCrud.InputParameters();
              parInp.P_ID_KRMH_NEG = this.ID_KRMH_NEG;
              parInp.P_ID_KRMH_NEG_RWYT_PENGGUNA = (this.puPengelola.Status == "input") ? 0 : this.selectedData.ID_KRMH_NEG_RWYT_PENGGUNA;
              parInp.P_ID_KRMH_NEG_RWYT_PENGGUNASpecified = true;
              ...
              parInp.P_NILAI = konfigApp.DecimaltoNull((decimal?)this.puPengelola.teNilaiPenetapan.Value);
              ...
              this.svcRiwayatPengelolaCrud = new SvcRumahRpengelolaCrud.call_pttClient();
        ...
        public delegate void UbahDsDetail(SvcRumahRpengelolaCrud.OutputParameters outCrud);

        public void ubahDsDetail(SvcRumahRpengelolaCrud.OutputParameters outCrud)
        {
            SvcRwyPggRmhNgrSelect.BPSIMANSROW_KRMH_NEG_RWYT_PENGGUNA dataPenyama = new SvcRwyPggRmhNgrSelect.BPSIMANSROW_KRMH_NEG_RWYT_PENGGUNA();
            ...
            dataPenyama.ID_KRMH_NEG_RWYT_PENGGUNA = (this.modeCrud == 'U') ? selectedData.ID_KRMH_NEG_RWYT_PENGGUNA : outCrud.PO_ID_KRMH_NEG_RWYT_PENGGUNA;
            dataPenyama.ID_KRMH_NEG_RWYT_PENGGUNASpecified = true;
            ...
            this.ID_KRMH_NEG_RWYT_PENGGUNA = (this.StatusCrud == "input") ? outCrud.PO_ID_KRMH_NEG_RWYT_PENGGUNA : selectedData.ID_KRMH_NEG_RWYT_PENGGUNA;
            ...
            switch (this.modeCrud)
            {
                case 'C':
                    ...
                    if (this.puPengelola.FilePath != null)
                    {
                        ...
                        simpanFile("ID_KRMH_NEG_RWYT_PENGGUNA", dataPenyama.ID_KRMH_NEG_RWYT_PENGGUNA, "M_KRMH_NEG_RWYT_PENGGUNA", filePath, "C", puPengelola.ID_JNSDOK);
                    }
                    else
                    {
                        ...
                        this.getById = true;
                        this.getInitTnhRPgn(" ID_KRMH_NEG_RWYT_PENGGUNA = " + this.ID_KRMH_NEG_RWYT_PENGGUNA.ToString());
                    }
                ...
                case 'U':
                    ...
                    if (this.puPengelola.FilePath != null)
                    {
                        ...
                        simpanFile("ID_KRMH_NEG_RWYT_PENGGUNA", dataPenyama.ID_KRMH_NEG_RWYT_PENGGUNA, "M_KRMH_NEG_RWYT_PENGGUNA", filePath, SELECT, puPengelola.ID_JNSDOK);
                    }
                    else
                    {
                        this.puPengelola.Close();
        ...
        protected override void bbHapus_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
             ...
             MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    parInp.P_ID_KRMH_NEGSpecified = true;
                    parInp.P_ID_KRMH_NEG_RWYT_PENGGUNA = selectedData.ID_KRMH_NEG_RWYT_PENGGUNA;
                    parInp.P_ID_KRMH_NEG_RWYT_PENGGUNASpecified = true;


                    parInp.P_SELECT = "D";
                    this.modeCrud = Convert.ToChar(parInp.P_SELECT);
                    svcRiwayatPengelolaCrud = new SvcRumahRpengelolaCrud.call_pttClient();
                    svcRiwayatPengelolaCrud.Open();
