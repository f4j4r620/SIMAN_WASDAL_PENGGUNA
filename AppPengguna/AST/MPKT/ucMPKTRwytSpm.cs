...
{
    class ucMPKTRwytSpm : UserControlDetail
    {
        private SvcMPKTSpmSelect.call_pttClient fetchData;
        private SvcMPKTSpmSelect.InputParameters parInp;
        private SvcMPKTSpmSelect.OutputParameters outData;
        public SvcMPKTSpmSelect.BPSIMANSROW_M_KKTIK_RWYT_SPM selectedData;
        ...
        public ucMPKTRwytSpm(decimal? _ID_KKTIK, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
            this.IdKspm = _ID_KKTIK;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcMPKTSpmSelect.BPSIMANSROW_M_KKTIK_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcMPKTSpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KKTIK = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcMPKTSpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMPKTSpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcMPKTSpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[i].TGL_BUKU).Substring(0, 10);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[i].TGL_SP2D).Substring(0, 10);

              if (date1 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[i].TGL_BUKU = null;
              }
              if (date2 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[i].TGL_SP2D = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KKTIK_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMPKTSpmSelect.BPSIMANSROW_M_KKTIK_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
