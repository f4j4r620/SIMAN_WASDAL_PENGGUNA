...
{
    class ucMPKTRwytMutasi : UserControlDetail
    {
        ...
        private SvcMPKTRwytMutasi.call_pttClient fetchData;
        private SvcMPKTRwytMutasi.InputParameters parInp;
        private SvcMPKTRwytMutasi.OutputParameters outData;
        public SvcMPKTRwytMutasi.BPSIMANSROW_KKTIK_RWYT_MUTASI selectedData;



        public ucMPKTRwytMutasi(decimal? _ID_KKTIK, String Status)
            : base()
        {
            this.initTnhRMutasi();
            this.Status = Status;
        ...
        private void initTnhRMutasi()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcMPKTRwytMutasi.BPSIMANSROW_KKTIK_RWYT_MUTASI);
        ...
        protected override void ucDetail_Load(object sender, EventArgs e)
        {
            this.search = "";
            this.initGrid();
            this.getInitMpktMutasi();
            gvUcDtl.OptionsBehavior.CopyToClipboardWithColumnHeaders = false;
        }

        #region load data
        public void getInitMpktMutasi(string _where = null)
        {
            myThread = new Thread(new ThreadStart(this.ShowProgresBar));
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcMPKTRwytMutasi.InputParameters();
            ...
            parInp.P_SORT = "ASC";
            parInp.STR_WHERE = String.Format(" ID_KKTIK = {0} {1}", this.ID_KKTIK, this.search);
            fetchData = new SvcMPKTRwytMutasi.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMPKTRwytMutasi.OutputParameters dataOut);

        public void showData(SvcMPKTRwytMutasi.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI.Count();
            ...
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 10);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 10);

              if (date1 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[i].TGL_BUKU = null;
              }
              if (date2 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[i].TGL_DSR_MUTASI = null;
              }
                binder.Add(serviceOutPut.SF_ROW_KKTIK_RWYT_MUTASI[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMPKTRwytMutasi.BPSIMANSROW_KKTIK_RWYT_MUTASI)selectedView.GetRow(e.FocusedRowHandle);
        ...
        protected override void bbSearch_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                this.getInitMpktMutasi(get_where_clause(nama_kolom, opr, parameter, parameter_2));
