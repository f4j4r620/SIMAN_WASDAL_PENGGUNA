...
{
    class ucMPKTRwytSusut : UserControlDetail
    {
        private SvcMPKTSusutSelect.call_pttClient fetchData;
        private SvcMPKTSusutSelect.InputParameters parInp;
        private SvcMPKTSusutSelect.OutputParameters outData;
        public SvcMPKTSusutSelect.BPSIMANSROW_M_KKTIK_RWYT_SUSUT selectedData;
        ...
        public ucMPKTRwytSusut(decimal? _ID_KKTIK, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcMPKTSusutSelect.BPSIMANSROW_M_KKTIK_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcMPKTSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            fetchData = new SvcMPKTSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMPKTSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcMPKTSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);

              if (date1 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[i].TGL_SUSUT = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KKTIK_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMPKTSusutSelect.BPSIMANSROW_M_KKTIK_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
