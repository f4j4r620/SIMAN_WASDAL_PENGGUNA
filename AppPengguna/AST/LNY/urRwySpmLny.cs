...
{
    class urRwySpmLny : UserControlDetail
    {
        private SvcLnySpmSelect.call_pttClient fetchData;
        private SvcLnySpmSelect.InputParameters parInp;
        private SvcLnySpmSelect.OutputParameters outData;
        public SvcLnySpmSelect.BPSIMANSROW_M_KLAIN_RWYT_SPM selectedData;
        ...
        public urRwySpmLny(decimal? _ID_KLAIN, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
            this.IdKspm = _ID_KLAIN;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcLnySpmSelect.BPSIMANSROW_M_KLAIN_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcLnySpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KLAIN = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcLnySpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcLnySpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcLnySpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i].TGL_BUKU).Substring(0, 10);
                string date2 = Convert.ToString(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i].TGL_SP2D).Substring(0, 10);
                if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i].TGL_BUKU = null;
                }
                if (date2 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i].TGL_SP2D = null;
                }

                this.binder.Add(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i]);
            }

            for (int i = 0; i < jmlDataGroup; i++)
            {
                binder.Add(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcLnySpmSelect.BPSIMANSROW_M_KLAIN_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
