...
{
    class urRwySusutLny : UserControlDetail
    {
        private SvcLnySusutSelect.call_pttClient fetchData;
        private SvcLnySusutSelect.InputParameters parInp;
        private SvcLnySusutSelect.OutputParameters outData;
        public SvcLnySusutSelect.BPSIMANSROW_M_KLAIN_RWYT_SUSUT selectedData;
        ...
        public urRwySusutLny(decimal? _ID_KLAIN, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
            this.IdKlny = _ID_KLAIN;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcLnySusutSelect.BPSIMANSROW_M_KLAIN_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcLnySusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KLAIN = {0} {1}", this.IdKlny, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcLnySusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcLnySusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcLnySusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
                if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[i].TGL_SUSUT = null;
                }

                this.binder.Add(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[i]);
            }

            for (int i = 0; i < jmlDataGroup; i++)
            {
                binder.Add(serviceOutPut.SF_ROW_M_KLAIN_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcLnySusutSelect.BPSIMANSROW_M_KLAIN_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
