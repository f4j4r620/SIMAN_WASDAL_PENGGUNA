...
{
    class UcRwyPggLny : UserControlDetail
    {


        private frmPengelola puPengelola;
        private String Status;
        public decimal? ID_KLAIN_RWYT_PENGGUNA;
        private decimal? ID_KLAIN;
        private string search;
        public frmProgres progresBar = null;

        private SvcLnyPggSelect.call_pttClient fetchData;
        private SvcLnyPggSelect.InputParameters parInp;
        private SvcLnyPggSelect.OutputParameters outData;
        public SvcLnyPggSelect.BPSIMANSROW_KLAIN_RWYT_PENGGUNA selectedData;



        public UcRwyPggLny(decimal? _ID_KLAIN, String Status)
            : base()
        {
        ...
        private void initTnhRPengguna()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcLnyPggSelect.BPSIMANSROW_KLAIN_RWYT_PENGGUNA);
        ...
        protected void view_dokumen(object sender, ItemClickEventArgs e)
        {
            try
            {
                ...
                parInp.P_ID = selectedData.ID_KLAIN_RWYT_PENGGUNA;
                parInp.P_IDSpecified = true;
                parInp.P_ID_TABLE = "ID_KLAIN_RWYT_PENGGUNA";
                parInp.P_TABLE = "M_KLAIN_RWYT_PENGGUNA";
        ...
        public void getInitTnhRPgn(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcLnyPggSelect.InputParameters();
            parInp.P_COL = "";
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KLAIN = {0} {1}", this.ID_KLAIN, _where);
            fetchData = new SvcLnyPggSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcLnyPggSelect.OutputParameters dataOut);
        private string StatusCrud = "";
        private decimal? NUM;
        public void showData(SvcLnyPggSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA.Count();

            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date1 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_KONTRAK).Substring(0, 10);
                string date2 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_MULAI).Substring(0, 10);
                string date3 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_SELESAI).Substring(0, 10);
                string date4 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_SK).Substring(0, 10);
                if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_KONTRAK = null;
                }
                if (date2 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_MULAI = null;
                }
                if (date3 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_SELESAI = null;
                }
                if (date4 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i].TGL_SK = null;
                }

                this.binder.Add(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i]);
            }

            for (int i = 0; i < jmlDataGroup; i++)
            {
                binder.Add(serviceOutPut.SF_ROW_KLAIN_RWYT_PENGGUNA[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcLnyPggSelect.BPSIMANSROW_KLAIN_RWYT_PENGGUNA)selectedView.GetRow(e.FocusedRowHandle);
        ...
        SvcLnyRpengelolaCrud.call_pttClient svcRiwayatPengelolaCrud;
        SvcLnyRpengelolaCrud.OutputParameters outRiwayatPengelolaCrud;
        ...
        private void saveForm(string text)
        {
            ...
            try
            {
                ...
                SvcLnyRpengelolaCrud.InputParameters parInp = new SvcLnyRpengelolaCrud.InputParameters();
                parInp.P_ID_KLAIN = this.ID_KLAIN;
                parInp.P_ID_KLAIN_RWYT_PENGGUNA = (this.puPengelola.Status == "input") ? 0 : this.selectedData.ID_KLAIN_RWYT_PENGGUNA;
                parInp.P_ID_KLAIN_RWYT_PENGGUNASpecified = true;
                parInp.P_ID_KLAINSpecified = true;
                ...
                parInp.P_TGL_SK = this.puPengelola.teTglSKPenetapan.Text;
                ...
                this.svcRiwayatPengelolaCrud = new SvcLnyRpengelolaCrud.call_pttClient();
        ...
        public delegate void UbahDsDetail(SvcLnyRpengelolaCrud.OutputParameters outCrud);

        public void ubahDsDetail(SvcLnyRpengelolaCrud.OutputParameters outCrud)
        {
            SvcLnyPggSelect.BPSIMANSROW_KLAIN_RWYT_PENGGUNA dataPenyama = new SvcLnyPggSelect.BPSIMANSROW_KLAIN_RWYT_PENGGUNA();

            dataPenyama.NUM = 999;
            dataPenyama.NUMSpecified = true;
            dataPenyama.ID_ASET = (this.modeCrud == 'U') ? selectedData.ID_ASET : 0;
            dataPenyama.ID_ASETSpecified = true;
            dataPenyama.ID_KLAIN = (this.modeCrud == 'U') ? selectedData.ID_KLAIN : 0;
            dataPenyama.ID_KLAIN_RWYT_PENGGUNA = (this.modeCrud == 'U') ? selectedData.ID_KLAIN_RWYT_PENGGUNA : outCrud.PO_ID_KLAIN_RWYT_PENGGUNA;
            dataPenyama.ID_KLAIN_RWYT_PENGGUNASpecified = true;
            dataPenyama.ID_KLAINSpecified = true;
            ...
            this.ID_KLAIN_RWYT_PENGGUNA = (this.StatusCrud == "input") ? outCrud.PO_ID_KLAIN_RWYT_PENGGUNA : selectedData.ID_KLAIN_RWYT_PENGGUNA;
            ...
            switch (this.modeCrud)
            {
                case 'C':
                    ...
                    if (this.puPengelola.FilePath != null)
                    {
                        ...
                        simpanFile("ID_KLAIN_RWYT_PENGGUNA", dataPenyama.ID_KLAIN_RWYT_PENGGUNA, "M_KLAIN_RWYT_PENGGUNA", filePath, "C");
                    }
                    else
                    {
                        ...
                        this.getById = true;
                        this.getInitTnhRPgn(" ID_KLAIN_RWYT_PENGGUNA = " + this.ID_KLAIN_RWYT_PENGGUNA.ToString());
                    }
                ...
                case 'U':
                    ...
                    if (this.puPengelola.FilePath != null)
                    {
                        ...
                        simpanFile("ID_KLAIN_RWYT_PENGGUNA", dataPenyama.ID_KLAIN_RWYT_PENGGUNA, "M_KLAIN_RWYT_PENGGUNA", filePath, SELECT);
                    }
                    else
                    {
                        this.puPengelola.Close();
        ...
        public void simpanFile(string ID_TYPE, decimal? ID_VALUE, string TABLE_TYPE, string filePath, string SELECT)
        {
            ...
            try
            {
                ...
                inputData.P_KD_DOK = puPengelola.ID_JNSDOK;
        ...
        protected override void bbHapus_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
             ...
             MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    myThread.Start();
                    SvcLnyRpengelolaCrud.InputParameters parInp = new SvcLnyRpengelolaCrud.InputParameters();
                    parInp.P_ID_KLAIN = selectedData.ID_KLAIN;
                    parInp.P_ID_KLAINSpecified = true;
                    parInp.P_ID_KLAIN_RWYT_PENGGUNA = selectedData.ID_KLAIN_RWYT_PENGGUNA;
                    parInp.P_ID_KLAIN_RWYT_PENGGUNASpecified = true;


                    parInp.P_SELECT = "D";
                    this.modeCrud = Convert.ToChar(parInp.P_SELECT);
                    svcRiwayatPengelolaCrud = new SvcLnyRpengelolaCrud.call_pttClient();
                    svcRiwayatPengelolaCrud.Open();
