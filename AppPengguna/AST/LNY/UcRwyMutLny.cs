...
{
    class UcRwyMutLny : UserControlDetail
    {
        ...
        private SvcLnyMutSelect.call_pttClient fetchData;
        private SvcLnyMutSelect.InputParameters parInp;
        private SvcLnyMutSelect.OutputParameters outData;
        public SvcLnyMutSelect.BPSIMANSROW_KLAIN_RWYT_MUTASI selectedData;



        public UcRwyMutLny(decimal? _ID_KLAIN, String Status)
            : base()
        {
            this.initTnhRMutasi();
            this.Status = Status;
        ...
        private void initTnhRMutasi()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcLnyMutSelect.BPSIMANSROW_KLAIN_RWYT_MUTASI);
        ...
        public void getInitTnhRMutasi(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcLnyMutSelect.InputParameters();
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KLAIN = {0} {1}", this.ID_KLAIN, this.search);
            fetchData = new SvcLnyMutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcLnyMutSelect.OutputParameters dataOut);

        public void showData(SvcLnyMutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI.Count();
            ...
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                binder.Add(serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i]);
            }
            ...
            for (int i = 0; i < jmlDataGroup; i++)
            {
                string date1 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 10);
                string date2 = Convert.ToString(serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 10);
                if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i].TGL_BUKU = null;
                }
                if (date2 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i].TGL_DSR_MUTASI = null;
                }

                this.binder.Add(serviceOutPut.SF_ROW_KLAIN_RWYT_MUTASI[i]);
            }
        }
        #endregion
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcLnyMutSelect.BPSIMANSROW_KLAIN_RWYT_MUTASI)selectedView.GetRow(e.FocusedRowHandle);
