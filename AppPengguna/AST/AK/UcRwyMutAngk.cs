...
{
    public class UcRwyMutAngk : UserControlDetail
    {
        ...
        private SvcRwyMutAngkSelect.call_pttClient fetchData;
        private SvcRwyMutAngkSelect.InputParameters parInp;
        private SvcRwyMutAngkSelect.OutputParameters outData;
        public SvcRwyMutAngkSelect.BPSIMANSROW_KANGK_RWYT_MUTASI selectedData;



        public UcRwyMutAngk(decimal? _ID_KANGK, String Status)
            : base()
        {
            this.initTnhRMutasi();
            this.Status = Status;
        ...
        private void initTnhRMutasi()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcRwyMutAngkSelect.BPSIMANSROW_KANGK_RWYT_MUTASI);
        ...
        #region load data
        public void getInitMutasi(string _where = null)
        {
            myThread = new Thread(new ThreadStart(this.ShowProgresBar));
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcRwyMutAngkSelect.InputParameters();
            ...
            parInp.P_SORT = "ASC";
            parInp.STR_WHERE = String.Format(" ID_KANGK = {0} {1}", this.ID_KANGK, this.search);
            fetchData = new SvcRwyMutAngkSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcRwyMutAngkSelect.OutputParameters dataOut);

        public void showData(SvcRwyMutAngkSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI.Count();
            ...
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[i].TGL_BUKU).Substring(0, 10);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[i].TGL_DSR_MUTASI).Substring(0, 10);
              if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[i].TGL_BUKU = null;
                }

              if (date2 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[i].TGL_DSR_MUTASI = null;
                }

                binder.Add(serviceOutPut.SF_ROW_KANGK_RWYT_MUTASI[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcRwyMutAngkSelect.BPSIMANSROW_KANGK_RWYT_MUTASI)selectedView.GetRow(e.FocusedRowHandle);
        ...
        protected override void bbSearch_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                this.getInitMutasi(get_where_clause(nama_kolom, opr, parameter, parameter_2));
