...
{
    class ucRsusutSenjata : UserControlDetail
    {
        private SvcSenjataSusutSelect.call_pttClient fetchData;
        private SvcSenjataSusutSelect.InputParameters parInp;
        private SvcSenjataSusutSelect.OutputParameters outData;
        public SvcSenjataSusutSelect.BPSIMANSROW_M_KSENJ_RWYT_SUSUT selectedData;
        ...
        public ucRsusutSenjata(decimal? _ID_KSENJ, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcSenjataSusutSelect.BPSIMANSROW_M_KSENJ_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcSenjataSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            fetchData = new SvcSenjataSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcSenjataSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcSenjataSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KSENJ_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KSENJ_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KSENJ_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KSENJ_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                binder.Add(serviceOutPut.SF_ROW_M_KSENJ_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcSenjataSusutSelect.BPSIMANSROW_M_KSENJ_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
