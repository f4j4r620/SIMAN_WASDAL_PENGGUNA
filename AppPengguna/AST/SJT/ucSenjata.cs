...
{
    class ucSenjata : UserControlDetail
    {
        private SvcSenjataSelect.call_pttClient fetchData;
        public SvcSenjataSelect.OutputParameters outDat;
        public SvcSenjataSelect.BPSIMANSROW_KSENJ selectedData;
        ...
        SvcSenjataSelect.BPSIMANSROW_KSENJ newRow;
        private SvcSenjataCrud.call_pttClient SvcSenjataCrud;
        ...
        public ucSenjata(bool tracking = false)
        {
            FROM_TRACKING = tracking;
            this.binder = new BindingSource();
            this.binder.DataSource = typeof(SvcSenjataSelect.BPSIMANSROW_KSENJ);
        ...
        public void hapusData(SvcSenjataSelect.BPSIMANSROW_KSENJ selectedData)
        {
            
            try
            {
                this.nonAktifForm("");
                //MessageBox.Show(selectedData.ID_ASET.ToString(),selectedData.ID_KSENJ.ToString());
              //  myThread = new Thread(new ThreadStart(ShowProgresBar),"");
               // myThread.Start();
                ...
                SvcSenjataCrud = new SvcSenjataCrud.call_pttClient();
                SvcSenjataCrud.Open();
                SvcSenjataCrud.Beginexecute(parInp, new AsyncCallback(getResultCrud), "");
            }
            catch
            {
                this.modeCrud = 'A';
        ...
        private void getResultCrud(IAsyncResult result)
        {
            try
            {
                outDataCrud = SvcSenjataCrud.Endexecute(result);
                SvcSenjataCrud.Close();
                this.Invoke(new closeProgresBar(this.CloseProgresBar),DevExpress.XtraBars.BarItemVisibility.Never);
                this.Invoke(new AktifkanForm(this.aktifkanForm), "");
                this.msg_get_database(outDataCrud.PO_RESULT, "Y", 0);
                if (String.Compare(outDataCrud.PO_RESULT.Trim(), "Y", true) == 0)
                {
                    this.Invoke(new UbahSenjata(this.ubahSenjata), outDataCrud);
                }
            }
            catch
            {
        ...
        private delegate void UbahSenjata(SvcSenjataCrud.OutputParameters dataOutCrud);

        private void ubahSenjata(SvcSenjataCrud.OutputParameters dataOutCrud)
        {
            SvcSenjataSelect.BPSIMANSROW_KSENJ dataPenyama = new SvcSenjataSelect.BPSIMANSROW_KSENJ();
            dataPenyama.ID_KSENJ = dataOutCrud.PO_ID_KSENJ;
        ...
        protected override void gvUcDtl_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            newRow = (SvcSenjataSelect.BPSIMANSROW_KSENJ)selectedView.GetRow(e.RowHandle);
        ...
        protected override void bbSearch_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                this.dataInisial = true;
                this.pencarian = true;
                this.getinitSenjata(get_where_clause(nama_kolom, opr, parameter, parameter_2));
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcSenjataSelect.BPSIMANSROW_KSENJ)selectedView.GetRow(e.FocusedRowHandle);
        ...
        public void getinitSenjata(string strwhere = null)
        {
            this.nonAktifForm("");
            ...
            fetchData = new SvcSenjataSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcSenjataSelect.OutputParameters dataOut);

        ucSenjataForm SenjataForm;


        public void showData(SvcSenjataSelect.OutputParameters serviceOutPut)
        {
            if (!FROM_TRACKING)
            {
                ...
                if (this.dataInisial == true)
                {
                    ...
                    if (jmlData > 0)
                    {
                        ...
                        StrTotalDb.Caption = serviceOutPut.SF_ROW_MKSENJ[0].TOTAL_DATA.ToString();
                    }
                    else
                    {
                ...
                if (jmlData > 0)
                {
                    this.SUM_RPH_ASET = serviceOutPut.SF_ROW_MKSENJ[0].SUM_RPH_ASET;
                    this.SUM_RPH_MUTASI = serviceOutPut.SF_ROW_MKSENJ[0].SUM_RPH_MUTASI;
                    this.SUM_RPH_SUSUT = serviceOutPut.SF_ROW_MKSENJ[0].SUM_RPH_SUSUT;
                }

                for (int i = 0; i < jmlData; i++)
                {
                    this.binder.Add(serviceOutPut.SF_ROW_MKSENJ[i]);
                }
            ...
            else
            {
                 ...
                 if (jmlData > 0)
                 {
                     SenjataForm = new ucSenjataForm("detail");
                     SenjataForm.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                     SenjataForm.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);

                     SenjataForm.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                     SenjataForm.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);
                     SenjataForm.Dock = System.Windows.Forms.DockStyle.Fill;
                     SenjataForm.IsiForm(serviceOutPut.SF_ROW_MKSENJ[0]);
                 }
                 else
                 {
                     MessageBox.Show("Data aset tidak ditemukan", "Perhatian");
                 }
                setPanel(SenjataForm);
               
                //this.frmKoorSatker.panelKoorSatker.Controls.Clear();
                //this.frmKoorSatker.panelKoorSatker.Controls.Add(SenjataForm);
            }
