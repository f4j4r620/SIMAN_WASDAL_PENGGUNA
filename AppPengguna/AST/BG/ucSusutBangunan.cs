...
{
    class ucSusutBangunan : UserControlDetail
    {
        private SvcSusutBangunanSelect.call_pttClient fetchData;
        private SvcSusutBangunanSelect.InputParameters parInp;
        private SvcSusutBangunanSelect.OutputParameters outData;
        public SvcSusutBangunanSelect.BPSIMANSROW_M_KBDG_RWYT_SUSUT selectedData;
        ...
        public ucSusutBangunan(decimal? _ID_KBDG, String status) 
            :base ()
        {
            this.initSusut();
            this.Status = Status;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcSusutBangunanSelect.BPSIMANSROW_M_KBDG_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null) 
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcSusutBangunanSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            fetchData = new SvcSusutBangunanSelect.call_pttClient();
            fetchData.Open();
        ...
        protected void getResult(IAsyncResult result)
        {
         ...
         private delegate void ShowData(SvcSusutBangunanSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcSusutBangunanSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date = Convert.ToString(serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
              if (date == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[i].TGL_SUSUT = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KBDG_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcSusutBangunanSelect.BPSIMANSROW_M_KBDG_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
