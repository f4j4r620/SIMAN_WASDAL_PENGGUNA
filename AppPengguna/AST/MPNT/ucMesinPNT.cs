...
{
    class ucMesinPNT : UserControlDetail
    {
        private SvcMesinPntSelect.call_pttClient fetchData;
        public SvcMesinPntSelect.OutputParameters outDat;
        public SvcMesinPntSelect.BPSIMANSROW_KALB selectedData;
        ...
        SvcMesinPntSelect.BPSIMANSROW_KALB newRow;
        private SvcMesinPntCrud.call_pttClient svcMPNTCrud;
        ...
        public ucMesinPNT(bool tracking = false)
        {
            FROM_TRACKING = tracking;
            this.binder = new BindingSource();
            this.binder.DataSource = typeof(SvcMesinPntSelect.BPSIMANSROW_KALB);
        ...
        public void hapusData(SvcMesinPntSelect.BPSIMANSROW_KALB selectedData)
        {
            ...
            try
            {
              ...
              //  myThread = new Thread(new ThreadStart(ShowProgresBar),"");
               // myThread.Start();
                ...
                svcMPNTCrud = new SvcMesinPntCrud.call_pttClient();
                svcMPNTCrud.Beginexecute(parInp, new AsyncCallback(getResultCrud), "");
            }
            catch
            {
                this.modeCrud = 'A';
        ...
        private void getResultCrud(IAsyncResult result)
        {
            try
            {
                outDataCrud = svcMPNTCrud.Endexecute(result);
                svcMPNTCrud.Close();
                this.Invoke(new closeProgresBar(this.CloseProgresBar),DevExpress.XtraBars.BarItemVisibility.Never);
                this.Invoke(new AktifkanForm(this.aktifkanForm), "");
                ...
                {
                    this.Invoke(new UbahMPNT(this.ubahMPNT), outDataCrud);
                }
            }
            catch
            {
        ...
        private delegate void UbahMPNT(SvcMesinPntCrud.OutputParameters dataOutCrud);

        private void ubahMPNT(SvcMesinPntCrud.OutputParameters dataOutCrud)
        {
            SvcMesinPntSelect.BPSIMANSROW_KALB dataPenyama = new SvcMesinPntSelect.BPSIMANSROW_KALB();
            dataPenyama.ID_KALB = dataOutCrud.PO_ID_KALB;
        ...
        protected override void gvUcDtl_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            newRow = (SvcMesinPntSelect.BPSIMANSROW_KALB)selectedView.GetRow(e.RowHandle);
        ...
        protected override void bbSearch_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                this.dataInisial = true;
                this.getinitMPNT(get_where_clause(nama_kolom, opr, parameter, parameter_2));
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMesinPntSelect.BPSIMANSROW_KALB)selectedView.GetRow(e.FocusedRowHandle);
        ...
        public void getinitMPNT(string strwhere = null)
        {
            this.nonAktifForm("");
            ...
            fetchData = new SvcMesinPntSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMesinPntSelect.OutputParameters dataOut);

        ucMesinPNTForm MPNTForm;


        public void showData(SvcMesinPntSelect.OutputParameters serviceOutPut)
        {
            if (!FROM_TRACKING)
            {
                ...
                if (this.dataInisial == true)
                {
                    ...
                    if (jmlData > 0)
                    {
                        ...
                        StrTotalDb.Caption = serviceOutPut.SF_ROW_MKALB[0].TOTAL_DATA.ToString();
                    }
                    else
                    {
                ...
                if (jmlData > 0)
                {
                    this.SUM_RPH_ASET = serviceOutPut.SF_ROW_MKALB[0].SUM_RPH_ASET;
                    this.SUM_RPH_MUTASI = serviceOutPut.SF_ROW_MKALB[0].SUM_RPH_MUTASI;
                    this.SUM_RPH_SUSUT = serviceOutPut.SF_ROW_MKALB[0].SUM_RPH_SUSUT;
                }

                for (int i = 0; i < jmlData; i++)
                {
                  string date1 = Convert.ToString(serviceOutPut.SF_ROW_MKALB[i].TGL_BUKU).Substring(0, 10);
                  string date2 = Convert.ToString(serviceOutPut.SF_ROW_MKALB[i].TGL_DANA).Substring(0, 10);
                  string date3 = Convert.ToString(serviceOutPut.SF_ROW_MKALB[i].TGL_PERLH).Substring(0, 10);
                  string date4 = Convert.ToString(serviceOutPut.SF_ROW_MKALB[i].TGL_REKAM).Substring(0, 10);
                  if (date1 == "11/11/1000")
                  {
                    serviceOutPut.SF_ROW_MKALB[i].TGL_BUKU = null;
                  }
                  if (date2 == "11/11/1000")
                  {
                    serviceOutPut.SF_ROW_MKALB[i].TGL_DANA = null;
                  }
                  if (date3 == "11/11/1000")
                  {
                    serviceOutPut.SF_ROW_MKALB[i].TGL_PERLH = null;
                  }
                  if (date4 == "11/11/1000")
                  {
                    serviceOutPut.SF_ROW_MKALB[i].TGL_REKAM = null;
                  }
                    this.binder.Add(serviceOutPut.SF_ROW_MKALB[i]);
                }
            ...
            else
            {
                ...
                if (jmlData > 0)
                {
                    MPNTForm = new ucMesinPNTForm("detail");

                    MPNTForm.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                    MPNTForm.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);

                    MPNTForm.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                    MPNTForm.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);
                    MPNTForm.Dock = System.Windows.Forms.DockStyle.Fill;
                    MPNTForm.IsiForm(serviceOutPut.SF_ROW_MKALB[0]);
                }
                else
                {
                ...
                //this.frmKoorSatker.panelKoorSatker.Controls.Add(MPNTForm);
            }
