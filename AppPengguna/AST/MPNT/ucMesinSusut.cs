...
{
    class ucMesinSusut : UserControlDetail
    {
        private SvcMesinPntSusutSelect.call_pttClient fetchData;
        private SvcMesinPntSusutSelect.InputParameters parInp;
        private SvcMesinPntSusutSelect.OutputParameters outData;
        public SvcMesinPntSusutSelect.BPSIMANSROW_M_KALB_RWYT_SUSUT selectedData;
        ...
        public ucMesinSusut(decimal? _ID_KALB, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcMesinPntSusutSelect.BPSIMANSROW_M_KALB_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcMesinPntSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            fetchData = new SvcMesinPntSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMesinPntSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcMesinPntSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date = Convert.ToString(serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
              if (date == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[i].TGL_SUSUT = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KALB_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMesinPntSusutSelect.BPSIMANSROW_M_KALB_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
