...
{
    class ucMesinSpm : UserControlDetail
    {
        private SvcMesinPntSmpSelect.call_pttClient fetchData;
        private SvcMesinPntSmpSelect.InputParameters parInp;
        private SvcMesinPntSmpSelect.OutputParameters outData;
        public SvcMesinPntSmpSelect.BPSIMANSROW_M_KALB_RWYT_SPM selectedData;
        ...
        public ucMesinSpm(decimal? _ID_KALB, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
            this.IdKspm = _ID_KALB;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcMesinPntSmpSelect.BPSIMANSROW_M_KALB_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcMesinPntSmpSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KALB = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcMesinPntSmpSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcMesinPntSmpSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcMesinPntSmpSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KALB_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[i].TGL_BUKU).Substring(0, 10);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[i].TGL_SP2D).Substring(0, 10);
              if (date1 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[i].TGL_BUKU = null;
              }
              if (date2 == "11/11/1000")
              {
                serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[i].TGL_SP2D = null;
              }
                binder.Add(serviceOutPut.SF_ROW_M_KALB_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcMesinPntSmpSelect.BPSIMANSROW_M_KALB_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
