...
{
    class ucTnhSpm : UserControlDetail
    {
        private SvcAssetTanahSpmSelect.call_pttClient fetchData;
        private SvcAssetTanahSpmSelect.InputParameters parInp;
        private SvcAssetTanahSpmSelect.OutputParameters outData;
        public SvcAssetTanahSpmSelect.BPSIMANSROW_M_KTNH_RWYT_SPM selectedData;
        ...
        public ucTnhSpm(decimal? _ID_KTNH, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcAssetTanahSpmSelect.BPSIMANSROW_M_KTNH_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcAssetTanahSpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            fetchData = new SvcAssetTanahSpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcAssetTanahSpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcAssetTanahSpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
              string date1 = Convert.ToString(serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[i].TGL_BUKU).Substring(0,10);
              string date2 = Convert.ToString(serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[i].TGL_SP2D).Substring(0,10);
                if (date1 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[i].TGL_BUKU = null;
                }
                if (date2 == "11/11/1000")
                {
                    serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[i].TGL_SP2D = null;
                }
                binder.Add(serviceOutPut.SF_ROW_M_KTNH_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcAssetTanahSpmSelect.BPSIMANSROW_M_KTNH_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
