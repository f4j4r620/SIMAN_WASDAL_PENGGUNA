...
{
    class UcATBRwySusut : UserControlDetail
    {
        private SvcATBSusutSelect.call_pttClient fetchData;
        private SvcATBSusutSelect.InputParameters parInp;
        private SvcATBSusutSelect.OutputParameters outData;
        public SvcATBSusutSelect.BPSIMANSROW_M_KTWJD_RWYT_SUSUT selectedData;
        ...
        public UcATBRwySusut(decimal? _ID_KTWJD, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
            this.IdKatb = _ID_KTWJD;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcATBSusutSelect.BPSIMANSROW_M_KTWJD_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcATBSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KTWJD = {0} {1}", this.IdKatb, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcATBSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcATBSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcATBSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KTWJD_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KTWJD_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KTWJD_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KTWJD_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                binder.Add(serviceOutPut.SF_ROW_M_KTWJD_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcATBSusutSelect.BPSIMANSROW_M_KTWJD_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
