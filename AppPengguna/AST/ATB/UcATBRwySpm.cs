...
{
    class UcATBRwySpm : UserControlDetail
    {
        private SvcATBSpmSelect.call_pttClient fetchData;
        private SvcATBSpmSelect.InputParameters parInp;
        private SvcATBSpmSelect.OutputParameters outData;
        public SvcATBSpmSelect.BPSIMANSROW_M_KTWJD_RWYT_SPM selectedData;
        ...
        public UcATBRwySpm(decimal? _ID_KTWJD, String status)
            : base()
        {
            this.initSpm();
            this.Status = Status;
            this.IdKspm = _ID_KTWJD;
        ...
        public void initSpm()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcATBSpmSelect.BPSIMANSROW_M_KTWJD_RWYT_SPM);
        ...
        public void getInitSpm(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcATBSpmSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KTWJD = {0} {1}", this.IdKspm, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcATBSpmSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcATBSpmSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcATBSpmSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_M_KTWJD_RWYT_SPM.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_M_KTWJD_RWYT_SPM[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_M_KTWJD_RWYT_SPM[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_M_KTWJD_RWYT_SPM[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                binder.Add(serviceOutPut.SF_ROW_M_KTWJD_RWYT_SPM[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcATBSpmSelect.BPSIMANSROW_M_KTWJD_RWYT_SPM)viewTerpilih.GetRow(e.FocusedRowHandle);
