...
{
    public delegate void UpdateATB(SvcATBCrud.OutputParameters dataOutATBCrud);
    public  class UcATBForm : ucDetailAsetForm
    {
      
        public ucIdentitasATB identitas = new ucIdentitasATB();
        private SvcATBSelect.BPSIMANSROW_KTWJD selectedData;
   
        public NonAktifkanFormSatker nonAktifForm;
        public AktifkanFormSatker aktifkanForm;
        public UpdateATB updateATB;
        ...
        private int ulang = 0;
        private SvcATBCrud.call_pttClient svcATBCrud;
        private SvcATBCrud.OutputParameters outDataCrud;
        private SvcATBPerkapSelect.call_pttClient fetchDataPerlengkapan;
        private SvcATBPerkapSelect.OutputParameters outDataPerlengkapanSelect;
        ...
        public UcATBForm(string status)
        {
        ...
        public bool IsiForm(SvcATBSelect.BPSIMANSROW_KTWJD Data)
        {
            this.selectedData = Data;
            try
            {
                this.ID_SATKER = selectedData.ID_SATKER;
                this.ID_KTWJD = selectedData.ID_KTWJD;
        ...
        private void getData()
        {
            ...
            try
            {
                ...
                parInp.P_SORT = "";
                parInp.STR_WHERE = "ID_KTWJD = " + this.ID_KTWJD;
                fetchDataPerlengkapan = new SvcATBPerkapSelect.call_pttClient();
            ...
            catch (Exception E)
            {
                ...
                this.CloseProgresBar(DevExpress.XtraBars.BarItemVisibility.Never);
                this.aktifkanForm("");
                MessageBox.Show("Galgal ambil data perlengkapan aset tak berwujud.", konfigApp.judulGagalAmbil);
        ...
        private delegate void ShowDataPerlengkapan(SvcATBPerkapSelect.OutputParameters dataOut);

        public void showDataPerlengkapan(SvcATBPerkapSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KTWJD_PERLENGKAP.Count();

            for (int i = 0; i < jmlDataGroup; i++)
            {
                this.identitas.binder.Add(serviceOutPut.SF_ROW_KTWJD_PERLENGKAP[i]);
            }
            this.identitas.gvPerlengkapan.BestFitColumns();
            this.identitas.gvPerlengkapan.OptionsView.ColumnAutoWidth= true;
            if (this.mulai == true)
            {
                this.getInitPhoto(1);
            }
        ...
        public void simpanUpdate()
        {
            if (this.cek_input())
            {
                    ...
                    try
                    {
                        ...
                        myThread.Start();
                        SvcATBCrud.InputParameters parInp = new SvcATBCrud.InputParameters();
                        ...
                        svcATBCrud = new SvcATBCrud.call_pttClient(konfigApp.SvcATBCrud_name,konfigApp.SvcATBCrud_address);
                        svcATBCrud.Open();
                        svcATBCrud.Beginexecute(parInp, new AsyncCallback(crudBangunan), "");
        ...
        private void crudBangunan(IAsyncResult result)
        {
            try
            {
                outDataCrud = svcATBCrud.Endexecute(result);
                svcATBCrud.Close();
                this.Invoke(new closeProgresBar(this.CloseProgresBar), DevExpress.XtraBars.BarItemVisibility.Never);
        ...
        private delegate void SimpanData(SvcATBCrud.OutputParameters dataOutATBCrud);

        private void simpanData(SvcATBCrud.OutputParameters dataOutATBCrud)
        {
            this.updateATB(dataOutATBCrud);
        }
        ...
        protected override void On_Load(object sender, EventArgs e)
        {
            ...
            this.TabDokumen.Text = "Perlengkapan";
        ...
        protected override void TabDetail_SelectedPageChanged(object sender, DevExpress.XtraTab.TabPageChangedEventArgs e)
        {
            ...
            if (TabDetail.SelectedTabPage == this.TabDokumen && status_Perlengkapan == false)
            {
                status_Perlengkapan = true;
                UcATBPerlengkapan ucPerkap = new UcATBPerlengkapan(this.ID_KTWJD, this.status);
                ucPerkap.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                ucPerkap.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);
                ucPerkap.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                ucPerkap.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);

                ucPerkap.Dock = DockStyle.Fill;
                TabDokumen.Controls.Clear();
                TabDokumen.Controls.Add(ucPerkap);
            }
            ...
            else if (TabDetail.SelectedTabPage == this.TabRiwayatPemeliharaan && status_RiwayatPemeliharaan == false)
            {
                ...
                UcATBRwyPlr ucPlrLny = new UcATBRwyPlr(this.ID_KTWJD, this.status);
            ...
            else if (TabDetail.SelectedTabPage == this.TabRiwayatPenilaian && status_RiwayatPenilaian == false)
            {
                ...
                UcATBRwyNil ucNilLny = new UcATBRwyNil(this.ID_KTWJD, this.status);
            ...
            else if (TabDetail.SelectedTabPage == this.TabRiwayatMutasi && status_RiwayatMutasi == false)
            {
                ...
                UcATBRwyMut ucMutLny = new UcATBRwyMut(this.ID_KTWJD, this.status);
            ...
            else if (TabDetail.SelectedTabPage == this.TabRiwayatPengelolaan && status_RiwayatPengguna == false)
            {
                ...
                UcATBRwyPgg ucPggLny = new UcATBRwyPgg(this.ID_KTWJD, this.status);
            ...
            {
                status_RiwayatPengguna = true;
                UcATBRwySusut UcaTBRwySusut = new UcATBRwySusut(this.ID_KTWJD, this.status);
                UcaTBRwySusut.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                UcaTBRwySusut.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);
                UcaTBRwySusut.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                UcaTBRwySusut.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);
                UcaTBRwySusut.Dock = DockStyle.Fill;
                TabSusut.Controls.Clear();
                TabSusut.Controls.Add(UcaTBRwySusut);
            }
            else if (TabDetail.SelectedTabPage == this.TabSpm && status_Spm == false)
            {
                status_Spm = true;
                UcATBRwySpm ucATBRwySpm = new UcATBRwySpm(this.ID_KTWJD, this.status);
                ucATBRwySpm.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                ucATBRwySpm.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);
                ucATBRwySpm.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                ucATBRwySpm.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);
                ucATBRwySpm.Dock = DockStyle.Fill;
                TabSpm.Controls.Clear();
                TabSpm.Controls.Add(ucATBRwySpm);
            }
        ...
        protected void getInitPhoto(int max=0)
        {
            ...
            parInpPhoto.STR_WHERE = String.Format("ID_ASET = {0} ", this.ID_ASET);
