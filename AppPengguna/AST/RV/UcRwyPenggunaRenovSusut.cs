...
{
    class UcRwyPenggunaRenovSusut : UserControlDetail
    {
        private SvcRenovasiSusutSelect.call_pttClient fetchData;
        private SvcRenovasiSusutSelect.InputParameters parInp;
        private SvcRenovasiSusutSelect.OutputParameters outData;
        public SvcRenovasiSusutSelect.BPSIMANSROW_KRNV_RWYT_SUSUT selectedData;
        ...
        public UcRwyPenggunaRenovSusut(decimal? _ID_KRNV, String status)
            : base()
        {
            this.initSusut();
            this.Status = Status;
            this.IdKrnV = _ID_KRNV;
        ...
        public void initSusut()
        {
            ...
            this.binder.DataSource = typeof(AppPengguna.SvcRenovasiSusutSelect.BPSIMANSROW_KRNV_RWYT_SUSUT);
        ...
        public void getInitSusut(string _where = null)
        {
            ...
            myThread.Start();
            this.nonAktifForm("");
            parInp = new SvcRenovasiSusutSelect.InputParameters();
            parInp.P_COL = "";
            decimal Max, Min;
            ...
            parInp.P_SORT = "DESC";
            parInp.STR_WHERE = String.Format(" ID_KRNV = {0} {1}", this.IdKrnV, _where);
            parInp.P_COUNT = "Y";
            Console.WriteLine(parInp.STR_WHERE);
            fetchData = new SvcRenovasiSusutSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcRenovasiSusutSelect.OutputParameters dataOut);
        public decimal? NUM;
        private string StatusCrud = "";
        public void showData(SvcRenovasiSusutSelect.OutputParameters serviceOutPut)
        {
            int jmlDataGroup = serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT.Count();
            if (this.getById == true && jmlDataGroup > 0)
            {
                if (this.StatusCrud == "edit")
                {
                    serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[0].NUM = this.NUM;
                }
                else
                {
                    serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[0].NUM = binder.Count + 1;
                }
            }
            if (this.dataInisial == true)
            {
                ...
                if (jmlDataGroup > 0)
                {
                    ...
                    StrTotalDb.Caption = serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[0].TOTAL_DATA.ToString();
                }
                else
                {
            ...
            {
                string date1 = Convert.ToString(serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[i].TGL_SUSUT).Substring(0, 10);
                if (date1 == "11/11/1000") { serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[i].TGL_SUSUT = null; }
                binder.Add(serviceOutPut.SF_ROW_KRNV_RWYT_SUSUT[i]);
            }
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcRenovasiSusutSelect.BPSIMANSROW_KRNV_RWYT_SUSUT)viewTerpilih.GetRow(e.FocusedRowHandle);
