...
{
    class UcRenovMstr : UserControlDetail
    {
        private SvcRenovasiSelect.call_pttClient fetchData;
        public SvcRenovasiSelect.OutputParameters outDat;
        public SvcRenovasiSelect.BPSIMANSROW_KRNV selectedData;
        ...
        SvcRenovasiSelect.BPSIMANSROW_KRNV newRow;
        ...
        public UcRenovMstr(bool tracking = false)
        {
            FROM_TRACKING = tracking;
            this.binder = new BindingSource();
            this.binder.DataSource = typeof(SvcRenovasiSelect.BPSIMANSROW_KRNV);
            ...
            this.setKolom("Nama Barang", "UR_SSKEL", "UR_SSKEL", 5, true, 120, "string", false);
            this.setKolom("Tgl Perolehan", "TGL_PERLH", "TGL_PERLH", 6, true, 120, "date");
            this.setKolom("Kuantitas", "KUANTITAS", "KUANTITAS", 7, true, 120, "integer");
            this.setKolom("Nilai Buku", "NILAI_BUKU", "NILAI_BUKU", 8, true, 120, "number", false);
            this.setKolom("Kondisi", "UR_KONDISI", "UR_KONDISI", 9, true, 120, "string", false);
            this.setKolom("Merk/Tipe", "MERK", "MERK", 10, true);
            this.setKolom("Keterangan", "CATATAN", "CATATAN", 11, true, 120, "integer", false);
            this.setKolom("Asal Perolehan", "ASL_PERLH", "ASL_PERLH", 12, true, 120, "string", false);
            this.setKolom("No. Bukti Perolehan", "NO_DANA", "NO_DANA", 13, true, 100, "integer", false);
            this.setKolom("Tgl. Mutasi", "TGL_DANA", "TGL_DANA", 14, true, 100, "date", false);
            this.setKolom("Kode KPKNL", "KDKPKNL", "KDKPKNL", 15, true, 100, "integer", false);
            
            gridDoubleClick = true;
            show_record = true;
            this.ShowFooter(true);
            this.SetSummary(6, "TGL_PERLH", "Total", "T O T A L");
            this.SetSummary(8, "NILAI_BUKU", "TOT_NILAI_BUKU"); // SUM_RPH_ASET + SUM_RPH_MUTASI + SUM_RPH_SUSUT
        ...
        protected override void ucDetail_Load(object sender, EventArgs e)
        {
            ...
            //this.initGrid();
            //this.getinitRenovasi();

            
        }
        
   
        public void hapusData(SvcRenovasiSelect.BPSIMANSROW_KRNV selectedData)
        {
            
            try
            {
                this.nonAktifForm("");
                //MessageBox.Show(selectedData.ID_ASET.ToString(),selectedData.ID_KRNV.ToString());
              //  myThread = new Thread(new ThreadStart(ShowProgresBar),"");
               // myThread.Start();
                ...
                svcRenovasiCrud = new SvcRenovasiCrud.call_pttClient();
                svcRenovasiCrud.Open();
                svcRenovasiCrud.Beginexecute(parInp, new AsyncCallback(getResultCrud), "");
            }
            catch
            {
                this.modeCrud = 'A';
        ...
        private void getResultCrud(IAsyncResult result)
        {
            try
            {
                ...
                {
                    this.Invoke(new UbahRenovasi(this.ubahRenovasi), outDataCrud);
                }
            }
            catch
            {
        ...
        private delegate void UbahRenovasi(SvcRenovasiCrud.OutputParameters dataOutCrud);

        private void ubahRenovasi(SvcRenovasiCrud.OutputParameters dataOutCrud)
        {
            SvcRenovasiSelect.BPSIMANSROW_KRNV dataPenyama = new SvcRenovasiSelect.BPSIMANSROW_KRNV();
            dataPenyama.ID_KRNV = dataOutCrud.PO_ID_KRNV;
        ...
        protected override void gvUcDtl_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            newRow = (SvcRenovasiSelect.BPSIMANSROW_KRNV)selectedView.GetRow(e.RowHandle);
        ...
        protected override void bbSearch_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                this.dataInisial = true;
                this.pencarian = true;
                this.getinitRenovasi(get_where_clause(nama_kolom, opr, parameter, parameter_2));
        ...
        protected override void gvUcDtl_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                selectedData = (SvcRenovasiSelect.BPSIMANSROW_KRNV)selectedView.GetRow(e.FocusedRowHandle);
        ...
        public void getinitRenovasi(string strwhere = null)
        {
            this.nonAktifForm("");
            ...
            fetchData = new SvcRenovasiSelect.call_pttClient();
            fetchData.Open();
        ...
        private delegate void ShowData(SvcRenovasiSelect.OutputParameters dataOut);

        UcRenovFrm RenovasiForm;


        public void showData(SvcRenovasiSelect.OutputParameters serviceOutPut)
        {
            if (!FROM_TRACKING)
            {
                ...
                if (this.dataInisial == true)
                {
                    ...
                    if (jmlData > 0)
                    {
                        ...
                        StrTotalDb.Caption = serviceOutPut.SF_ROW_MKRNV[0].TOTAL_DATA.ToString();
                    }
                    else
                    {
                ...
                if (jmlData > 0)
                {
                    this.SUM_RPH_ASET = serviceOutPut.SF_ROW_MKRNV[0].SUM_RPH_ASET;
                    this.SUM_RPH_MUTASI = serviceOutPut.SF_ROW_MKRNV[0].SUM_RPH_MUTASI;
                    this.SUM_RPH_SUSUT = serviceOutPut.SF_ROW_MKRNV[0].SUM_RPH_SUSUT;
                }

                for (int i = 0; i < jmlData; i++)
                {
                    string date1 = Convert.ToString(serviceOutPut.SF_ROW_MKRNV[i].TGL_PERLH).Substring(0, 10);
                    if (date1 == "11/11/1000") { serviceOutPut.SF_ROW_MKRNV[i].TGL_PERLH = null; }
                    string date2 = Convert.ToString(serviceOutPut.SF_ROW_MKRNV[i].TGL_DANA).Substring(0, 10);
                    if (date2 == "11/11/1000") { serviceOutPut.SF_ROW_MKRNV[i].TGL_DANA = null; }
                    this.binder.Add(serviceOutPut.SF_ROW_MKRNV[i]);
                }
            ...
            else
            {
                 ...
                 if (jmlData > 0)
                 {
                     RenovasiForm = new UcRenovFrm("detail");
                     RenovasiForm.nonAktifForm = new NonAktifkanFormSatker(this.nonAktifForm);
                     RenovasiForm.aktifkanForm = new AktifkanFormSatker(this.aktifkanForm);

                     RenovasiForm.ShowProgresBar = new showProgresBar(this.ShowProgresBar);
                     RenovasiForm.CloseProgresBar = new closeProgresBar(this.CloseProgresBar);
                     RenovasiForm.Dock = System.Windows.Forms.DockStyle.Fill;
                     RenovasiForm.IsiForm(serviceOutPut.SF_ROW_MKRNV[0]);
                 }
                 else
                 {
                     MessageBox.Show("Data aset tidak ditemukan", "Perhatian");
                 }
                setPanel(RenovasiForm);
               
                //this.frmKoorSatker.panelKoorSatker.Controls.Clear();
                //this.frmKoorSatker.panelKoorSatker.Controls.Add(RenovasiForm);
            }
