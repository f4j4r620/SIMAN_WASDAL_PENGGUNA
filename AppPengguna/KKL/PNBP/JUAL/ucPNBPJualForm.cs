...
{
    public partial class ucPNBPJualForm : UserControl
    {
        public ToggleProgressBar toggleProgressBar;
        public SimpanDataRPNBP simpanDataRkmPnbpJual;
        ...
        public SvcWasdalPTJualPnbpSelect.WASDALSROW_SELECT_JUAL_PNBP dataTerpilih;
        public SvcWasdalJualSkSelect.WASDALSROW_READ_WASDAL_PT_JUAL skTerpilih;
        ...
        public ucPNBPJualForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        private void getAkun()
        {
            ...
            try
            {
                ...
                input.P_COL = "";
                input.P_SORT = "";
                input.STR_WHERE = " KDMAKMAP LIKE '423%'";
        ...
        KKL.TL.PU.frmPuSatker formPuSatker;
        KKL.TL.PU.frmPuKorwil formPuKorwil;
        KKL.TL.PU.frmPuEselon1 formPuEselon1;
        KKL.TL.PU.frmPuKl formPuKl;
        ...
        private void sbCariPemohon_Click(object sender, EventArgs e)
        {
            ...
            if (teJenisPemohon.Text.Trim() != "")
            {
                if (teJenisPemohon.Text == konfigApp.levelSatker)
                {
                    ...
                    {
                        formPuSatker = new KKL.TL.PU.frmPuSatker()
                        {
                ...
                else if (teJenisPemohon.Text == konfigApp.levelKorwil)
                {
                    ...
                    {
                        formPuKorwil = new KKL.TL.PU.frmPuKorwil()
                        {
                            toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
                            selectUnitKerja = new SelectDataUnitKerja(setUnitKerja),
                            whereAwal = preWhere
                ...
                else if (teJenisPemohon.Text == konfigApp.levelEselon1)
                {
                    ...
                    {
                        formPuEselon1 = new KKL.TL.PU.frmPuEselon1()
                        {
                            toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
                            selectUnitKerja = new SelectDataUnitKerja(setUnitKerja),
                            whereAwal = preWhere
                ...
                else if (teJenisPemohon.Text == konfigApp.levelKl)
                {
                    ...
                    if (formPuKl == null)
                    {
                        formPuKl = new KKL.TL.PU.frmPuKl()
                        {
                            toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
                            selectUnitKerja = new SelectDataUnitKerja(setUnitKerja),
                            whereAwal = preWhere
        ...
        {
            if (teNomorSk.Text.Trim() != "" && teNTPN.Text != "" && teKet.Text != "" && filePath!=null && teFileBukti.Text.Trim() != "" )
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                    simpanDataRkmPnbpJual(statusForm);
            }
        ...
        SvcWasdalPTJualAsetPnbpSelect.OutputParameters dOutAsetSewaPnbp; 
        SvcWasdalPTJualAsetPnbpSelect.execute_pttClient ambilDaftarAsetPnbp;
        SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalPTJualAsetPnbpSelect.InputParameters parInput = new SvcWasdalPTJualAsetPnbpSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                parInput.P_SORT = "";
                parInput.STR_WHERE = String.Format(" (SK_KEPUTUSAN = '{0}' AND NTPN = '{1}' ) ", teNomorSk.Text.Trim(),dataTerpilih.NTPN);
                ambilDaftarAsetPnbp = new SvcWasdalPTJualAsetPnbpSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalPTJualAsetPnbpSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalPTJualAsetPnbpSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_DTLSEL_WASDAL_JUAL_PNBP.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_DTLSEL_WASDAL_JUAL_PNBP[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i].IS_TB = (dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_JUAL_PNBP[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        JUAL.frmPuAset formDaftarAsetSatker;
        JUAL.frmPuAset formDaftarAsetKorwil;
        JUAL.frmPuAset formDaftarAsetEselon1;
        JUAL.frmPuAset formDaftarAsetKl;
        ...
        private void sbTambah_Click(object sender, EventArgs e)
        {
            if (teJenisPemohon.Text == konfigApp.levelSatker)
            {
                ...
                {
                    formDaftarAsetSatker = new JUAL.frmPuAset()
                    {
                        toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
        ...
        SvcWasdalPTJualPnbpDetilCud.OutputParameters dOutSimpanAset;
        SvcWasdalPTJualPnbpDetilCud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset,decimal? _nilaiTotalAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalPTJualPnbpDetilCud.InputParameters parInp = new SvcWasdalPTJualPnbpDetilCud.InputParameters();
                ...
                simpanDftrAset = new SvcWasdalPTJualPnbpDetilCud.execute_pttClient();
                //simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(SvcWasdalPTJualPnbpDetilCud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalPTJualPnbpDetilCud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP dataPenyama;
                        ...
                        {
                            SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP daftarAsetTerpilih = (SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP)daftarAsetSalinan[i];
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP daftarAsetTerpilih = (SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PENETAPAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS_SK;
        ...
        {
            SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalPTJualAsetPnbpSelect.WASDALSROW_DTLSEL_JUAL_PNBP)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void btnViewDok_Click(object sender, EventArgs e)
        {
            try 
            {
                ...
                parInp.P_NO_BUKTI_LAKSANA = teNTPN.Text;
        ...
        private void dsFileWasdal(SvcWasdalManfaatTdLanjutFile.OutputParameters outFileWasdal)
        {
            ...
            {
                KKL.TL.PU.FrmPuViewPdf FrmPuViewPdf = new KKL.TL.PU.FrmPuViewPdf();

                System.IO.File.WriteAllBytes("DOK_BUKTI_" + teNTPN.Text + ".pdf", outFileWasdal.PO_FILE_BUKTI);
                FrmPuViewPdf.display("DOK_BUKTI_" + teNTPN.Text + ".pdf");
