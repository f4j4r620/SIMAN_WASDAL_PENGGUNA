...
{
    public partial class ucBelumPSPBangunanPengelola : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA224.InputParameters input;
        SvcMonSatkerGunaA224.OutputParameters output;
        SvcMonSatkerGunaA224.execute_pttClient client;
        SvcMonSatkerGunaA224.WASDALSROW_MON_GUNA_SATKER_A224 rowData;

        public ucBelumPSPBangunanPengelola()
        {
            InitializeComponent();
        }

        public ucBelumPSPBangunanPengelola(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA224.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA224.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA224.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA224.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A224.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A224);
