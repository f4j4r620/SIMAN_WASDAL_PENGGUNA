...
{
    public partial class ucPNBPSewaBmnForm : UserControl
    {
        public ToggleProgressBar toggleProgressBar;
        public SimpanDataRPNBP simpanDataRkmPnbpSwBmn;
        ...
        public ucPNBPSewaBmnForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        private void sbSimpan_Click(object sender, EventArgs e)
        {
            if (teNomorSk.Text.Trim() != "" && teNTPN.Text != "" && teKet.Text != "" && filePath!=null && teFileBukti.Text.Trim() != "" )
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                    simpanDataRkmPnbpSwBmn(statusForm);
            }
        ...
        SvcWasdalManfaatAsetPnbpSelect.OutputParameters dOutAsetSewaPnbp;
        SvcWasdalManfaatAsetPnbpSelect.execute_pttClient ambilDaftarAsetPnbp;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                ambilDaftarAsetPnbp = new SvcWasdalManfaatAsetPnbpSelect.execute_pttClient();
        ...
        private void loadDataDaftarAset(SvcWasdalManfaatAsetPnbpSelect.OutputParameters outputDaftarAset)
        {
            ...
            if (jmlData >= 1 && outputDaftarAset.SF_DTLSEL_WASDAL_MANFAAT_PNBP[0].KD_BRG != "-")
            {
                ...
                {
                    dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i].IS_TB = (dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewaPnbp.SF_DTLSEL_WASDAL_MANFAAT_PNBP[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        SWBMN.frmPuAset formDaftarAsetKorwil;
        SWBMN.frmPuAset formDaftarAsetEselon1;
        SWBMN.frmPuAset formDaftarAsetKl;
            ...
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        {
                            SvcWasdalManfaatAsetPnbpSelect.WASDALSROW_DTLSEL_MANFAAT_PNBP daftarAsetTerpilih = (SvcWasdalManfaatAsetPnbpSelect.WASDALSROW_DTLSEL_MANFAAT_PNBP)daftarAsetSalinan[i];
                            dataPenyama = null;
