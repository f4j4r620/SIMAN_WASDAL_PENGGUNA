...
using AppPengguna.KKL.TL.PBKSL;
...
namespace AppPengguna.KKL.TL.PBKSL
{
    public partial class ucRTLPBKSLForm : UserControl
    {
        ...
        SvcWasdalHapusSelect.OutputParameters dOutAsetSewa;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                parInput.P_SORT = "";
                parInput.STR_WHERE = String.Format(" (SK_KEPUTUSAN = '{0}' AND NO_BUKTI_LAKSANA = '{1}' ) ", teNomorSk.Text.Trim(),(dataTerpilih.NO_BUKTI_LAKSANA==null)?teNoBuktiLaksana.Text : dataTerpilih.NO_BUKTI_LAKSANA);
        ...
        private void loadDataDaftarAset(SvcWasdalHapusSelect.OutputParameters outputDaftarAset)
        {
            ...
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_HAPUS[0].KD_BRG != "-")
            {
                ...
                {
                    dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i].IS_TB = (dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_ROW_WASDAL_HAPUS[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
