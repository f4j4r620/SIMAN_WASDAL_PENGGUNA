...
namespace AppPengguna.KSK.MWD.TUKAR.DETAIL1
{
    ...
    public partial class ucTindakLanjut : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTindakLanjut(FrmKoorSatker _frmSatker, ucTukar _ucTUKAR)
        {
            InitializeComponent();
        ...
        public void getTindakLanjut()
        {
            ...
            try
            {
                ...
                parInp.STR_WHERE = String.Format("STATUS_BMN_YN ='Y' AND (KD_SATKER LIKE '{0}%' OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={1}))", konfigApp.kodeSatker.Substring(0, 15), konfigApp.idSatker) + TUKAR.strKdPelayanan + this.strCari;
                parInp.P_COL = "KD_BRG,NUP";
                parInp.P_SORT = "ASC";
                parInp.P_COUNT = "y";
                svcMonWasdalAllSelect = new SvcWasdalPTMonBMNAll.execute_pttClient();
                svcMonWasdalAllSelect.Beginexecute(parInp, new AsyncCallback(getDataTLTUKAR), null);
            }
            catch
            {
        ...
        private void getDataTLTUKAR(IAsyncResult result)
        {
            try
            {
                ...
                frmSatker.fToggleProgressBar("finish");
                this.Invoke(new AktifkanForm(frmSatker.aktifkanForm), "");
                this.Invoke(new LoadDataTLTUKAR(this.loadDataTLTUKAR), outMonAllSelect);
            }
            catch
            {
                frmSatker.fToggleProgressBar("finish");
        ...
        private delegate void LoadDataTLTUKAR(SvcWasdalPTMonBMNAll.OutputParameters dataOut);

        private void loadDataTLTUKAR(SvcWasdalPTMonBMNAll.OutputParameters dataOut)
        {
        ...
        public void displayData()
        {
            ...
            if (dataInisial == true)
            {
                gcTLTUKAR.DataSource = null;
                gcTLTUKAR.DataSource = dsGrid;
            }
            else
            {
                gcTLTUKAR.RefreshDataSource();
            }
            gvTLTUKAR.BestFitColumns();
        }

        private void gvTLTUKAR_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                if (rowTerpilih.SelectedRowsCount > 0)
                {
                    ...
                    posisiRow = gvTLTUKAR.GetFocusedDataSourceRowIndex();
        ...
        frmTL frmTL;
        private void gvTLTUKAR_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (frmTL == null)
                {
                    frmTL = new frmTL(TUKAR);
                ...
                frmTL.AsetPenggantiAlamat = dataTerpilih.ASET_PENGGANTI_ALAMAT;
                frmTL.AsetPenggantiJenis = dataTerpilih.ASET_PENGGANTI_JENIS;
                frmTL.AsetPenggantiKuantitas = dataTerpilih.ASET_PENGGANTI_KUANTITAS;
                frmTL.AsetPenggantiLuas = dataTerpilih.ASET_PENGGANTI_LUAS;
