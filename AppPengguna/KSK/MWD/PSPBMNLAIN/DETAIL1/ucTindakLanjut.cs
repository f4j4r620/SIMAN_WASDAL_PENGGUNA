...
namespace AppPengguna.KSK.MWD.PSPBMNLAIN.DETAIL1
{
    ...
    public partial class ucTindakLanjut : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTindakLanjut(FrmKoorSatker _frmSatker, ucPSPBMNLAIN _ucPSPBMNLAIN)
        {
            InitializeComponent();
        ...
        public void getTindakLanjut()
        {
            ...
            try
            {
                ...
                //parInp.STR_WHERE = String.Format(" ID_SATKER = {0} ", konfigApp.idSatker) + " AND NO_SURAT IS NOT NULL " + PSPBMNLAIN.strKdPelayanan + this.strCari;
                parInp.STR_WHERE = String.Format("STATUS_BMN_YN ='Y' AND (KD_SATKER LIKE '{0}%' OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={1}))", konfigApp.kodeSatker.Substring(0, 15), konfigApp.idSatker) + " AND NO_SURAT IS NOT NULL " + PSPBMNLAIN.strKdPelayanan + this.strCari;
