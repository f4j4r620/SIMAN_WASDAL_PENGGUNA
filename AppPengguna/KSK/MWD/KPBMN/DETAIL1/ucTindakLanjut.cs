...
namespace AppPengguna.KSK.MWD.KPBMN.DETAIL1
{
    ...
    public partial class ucTindakLanjut : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTindakLanjut(FrmKoorSatker _frmSatker, ucKPBMN _ucASPBMN)
        {
            InitializeComponent();
        ...
        public void getTindakLanjut()
        {
            ...
            try
            {
                ...
                parInp.STR_WHERE = String.Format("STATUS_BMN_YN ='Y' AND (KD_SATKER LIKE '{0}%' OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={1}))", konfigApp.kodeSatker.Substring(0, 15), konfigApp.idSatker) + " AND NO_SURAT IS NOT NULL  " + KPBMN.strKdPelayanan + this.strCari;
                parInp.P_COL = "KD_BRG,NUP";
                parInp.P_SORT = "ASC";
                parInp.P_COUNT = "y";
                svcMonWasdalAllSelect = new SvcWasdalManfaatAllSelect.execute_pttClient();
                svcMonWasdalAllSelect.Beginexecute(parInp, new AsyncCallback(getDataTLKPBMN), null);
            }
            catch
            {
        ...
        private void getDataTLKPBMN(IAsyncResult result)
        {
            try
            {
                ...
                frmSatker.fToggleProgressBar("finish");
                this.Invoke(new AktifkanForm(frmSatker.aktifkanForm), "");
                this.Invoke(new LoadDataTLKPBMN(this.loadDataTLKPBMN), outMonAllSelect);
            }
            catch
            {
                frmSatker.fToggleProgressBar("finish");
        ...
        private delegate void LoadDataTLKPBMN(SvcWasdalManfaatAllSelect.OutputParameters dataOut);

        private void loadDataTLKPBMN(SvcWasdalManfaatAllSelect.OutputParameters dataOut)
        {
        ...
        public void displayData()
        {
            ...
            if (dataInisial == true)
            {
                gcTLKPBMN.DataSource = null;
                gcTLKPBMN.DataSource = dsGrid;
            }
            else
            {
                gcTLKPBMN.RefreshDataSource();
            }
            gvTLKPBMN.BestFitColumns();
        }

        private void gvTLKPBMN_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                if (rowTerpilih.SelectedRowsCount > 0)
                {
                    ...
                    posisiRow = gvTLKPBMN.GetFocusedDataSourceRowIndex();
        ...
        frmTL frmTL;
        private void gvTLKPBMN_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (frmTL == null)
                {
                    frmTL = new frmTL(KPBMN);
