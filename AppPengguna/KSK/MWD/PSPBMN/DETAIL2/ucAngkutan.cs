...
namespace AppPengguna.KSK.MWD.PSPBMN.DETAIL2
{
    ...
    public partial class ucAngkutan : DevExpress.XtraEditors.XtraUserControl
    {
        public CariDataOnline cariDataOnline;

        SvcMonAngkSelect.execute_pttClient svcAngkutanSelect = null;
        SvcMonAngkSelect.OutputParameters outAngkutanSelect = null;
        SvcMonAngkSelect.WASDALSROW_MON_BMN_PSP2 dataTerpilih = null;
        ...
        public ucAngkutan(FrmKoorSatker _frmKpknl, ucPSPBMN _ucPspBmn)
        {
            InitializeComponent();
        ...
        public void getAngkutan()
        {
            ...
            try
            {
                ...
                SvcMonAngkSelect.InputParameters parInp = new SvcMonAngkSelect.InputParameters();
                ...
                parInp.P_SORT = "ASC";
                parInp.P_COUNT = "y";
                svcAngkutanSelect = new SvcMonAngkSelect.execute_pttClient();
                svcAngkutanSelect.Beginexecute(parInp, new AsyncCallback(getDataAngkutan), null);
            }
            catch
            {
        ...
        private void getDataAngkutan(IAsyncResult result)
        {
            try
            {
                outAngkutanSelect = svcAngkutanSelect.Endexecute(result);
                svcAngkutanSelect.Close();
                frmKpknl.fToggleProgressBar("finish");
                this.Invoke(new AktifkanForm(frmKpknl.aktifkanForm), "");
                this.Invoke(new LoadDataAngkutan(this.loadDataAngkutan), outAngkutanSelect);
            }
            catch
            {
                frmKpknl.fToggleProgressBar("finish");
        ...
        private delegate void LoadDataAngkutan(SvcMonAngkSelect.OutputParameters dataOut);

        private void loadDataAngkutan(SvcMonAngkSelect.OutputParameters dataOut)
        {
            int jmlData = dataOut.SF_MON_KANGK_PSP2.Count();
            decimal jmlCurrent = 0;
            string totalData = (jmlData == 0) ? jmlData.ToString() : dataOut.SF_MON_KANGK_PSP2[jmlData - 1].TOTAL_DATA.ToString();
            ...
            {
                //dataOut.SF_MON_KANGK_PSP2[i].TGL_SK = konfigApp.setDefaultDate(dataOut.SF_MON_KANGK_PSP2[i].TGL_SK);
                dataOut.SF_MON_KANGK_PSP2[i].TGL_PEROLEHAN = konfigApp.setDefaultDate(dataOut.SF_MON_KANGK_PSP2[i].TGL_PEROLEHAN);
                dsGrid.Add(  dataOut.SF_MON_KANGK_PSP2[i]);
                this.adaData = true;
            }
            labelTotData.Text = "";
        ...
        private void gvAngkutan_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                {
                    dataTerpilih = (SvcMonAngkSelect.WASDALSROW_MON_BMN_PSP2)rowTerpilih.GetRow(e.FocusedRowHandle);
