...
namespace AppPengguna.KSK.MWD.BSG.DETAIL1
{
    ...
    public partial class ucTindakLanjut : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTindakLanjut(FrmKoorSatker _frmSatker, ucBSG _ucBSG)
        {
            InitializeComponent();
        ...
        public void getTindakLanjut()
        {
            ...
            try
            {
                ...
                parInp.STR_WHERE = String.Format("STATUS_BMN_YN ='Y' AND (KD_SATKER LIKE '{0}%' OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={1}))", konfigApp.kodeSatker.Substring(0, 15), konfigApp.idSatker) + "AND (NO_SURAT IS NOT NULL) " + BSG.strKdPelayanan + this.strCari; 
                parInp.P_COL = "KD_BRG,NUP";
                parInp.P_SORT = "ASC";
                parInp.P_COUNT = "y";
                svcMonWasdalAllSelect = new SvcWasdalManfaatAllSelect.execute_pttClient();
                svcMonWasdalAllSelect.Beginexecute(parInp, new AsyncCallback(getDataTLBSG), null);
            }
            catch
            {
        ...
        private void getDataTLBSG(IAsyncResult result)
        {
            try
            {
                ...
                frmSatker.fToggleProgressBar("finish");
                this.Invoke(new AktifkanForm(frmSatker.aktifkanForm), "");
                this.Invoke(new LoadDataTLBSG(this.loadDataTLBSG), outMonAllSelect);
            }
            catch
            {
                frmSatker.fToggleProgressBar("finish");
        ...
        private delegate void LoadDataTLBSG(SvcWasdalManfaatAllSelect.OutputParameters dataOut);

        private void loadDataTLBSG(SvcWasdalManfaatAllSelect.OutputParameters dataOut)
        {
        ...
        public void displayData()
        {
            ...
            if (dataInisial == true)
            {
                gcTLBSG.DataSource = null;
                gcTLBSG.DataSource = dsGrid;
            }
            else
            {
                gcTLBSG.RefreshDataSource();
            }
            gvTLBSG.BestFitColumns();
        }

        private void gvTLBSG_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                if (rowTerpilih.SelectedRowsCount > 0)
                {
                    ...
                    posisiRow = gvTLBSG.GetFocusedDataSourceRowIndex();
        ...
        frmTL frmTL;
        private void gvTLBSG_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (frmTL == null)
