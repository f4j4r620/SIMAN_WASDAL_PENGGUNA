...
{
    public partial class ucRskPmpForm : UserControl
    {
        ...
        public SvcWasdalPmpSkSelect.WASDALSROW_READ_WASDAL_PT_MODAL dataTerpilih;
        ...
        public ucRskPmpForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        public void inisialisasiForm()
        {
            ...
            //tePeriode.Properties.ReadOnly = false;
            ...
            switch (statusForm)
            {
                ...
                case "A":
                    ...
                    //tePeriode.Properties.ReadOnly = true;
        ...
        private void dsFileWasdal(SvcFileRekamSkCru.OutputParameters outFileWasdal)
        {
            if (outFileWasdal.PO_RESULT == "Y")
            {
                ...
                System.IO.File.WriteAllBytes(teFileSk.Text + ".pdf", outFileWasdal.PO_FILE_DOK);
                FrmPuViewPdf.display(teFileSk.Text + ".pdf");
                FrmPuViewPdf.ShowDialog();
            }
            else
            {
        ...
        SvcWasdalPmpSelect.OutputParameters dOutDaftarAset;
        SvcWasdalPmpSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL dataAsetTerpilih; 
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                ambilDaftarAset = new SvcWasdalPmpSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalPmpSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalPmpSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_ROW_WASDAL_PT_MODAL.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i].IS_TB = (outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i]);
                    nilaiPersetujuan = nilaiPersetujuan + outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + outputDaftarAset.SF_ROW_WASDAL_PT_MODAL[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        SvcWasdalPmpDetailCrud.OutputParameters dOutSimpanAset;
        SvcWasdalPmpDetailCrud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalPmpDetailCrud.InputParameters parInp = new SvcWasdalPmpDetailCrud.InputParameters();
                ...
                //{
                //    case 'U':
                //        parInp.P_KUANTITAS = Convert.ToDecimal(formPuPeneetapan.teKuantitas.Text);
                ...
                parInp.P_TGL_SK = teTanggalSk.Text;
                modeCudAset = _modeCudAset;
                parInp.P_SELECT = Convert.ToString(modeCudAset);
                simpanDftrAset = new SvcWasdalPmpDetailCrud.execute_pttClient();
                simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(SvcWasdalPmpDetailCrud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalPmpDetailCrud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL dataPenyama;
                        ...
                        for (int i = 0; i < daftarAsetSalinan.Count; i++)
                        {
                            ...
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL daftarAsetTerpilih = (SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL)daftarTerpilih[i];
        ...
        {
            SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void gvDaftarAset_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL daftarAsetTerpilih;
                daftarAsetTerpilih = (SvcWasdalPmpSelect.WASDALSROW_WASDAL_PT_MODAL)view.GetRow(info.RowHandle);
