...
{
    public partial class ucRskHibahForm : UserControl
    {
        ...
        public SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH dataTerpilih;
        ...
        public ucRskHibahForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        SvcWasdalHibahSelect.OutputParameters dOutDaftarAset; 
        SvcWasdalHibahSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH dataAsetTerpilih; 
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                ambilDaftarAset = new SvcWasdalHibahSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalHibahSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalHibahSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH.Count(); 
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i].IS_TB = (outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i]);
                    nilaiPersetujuan = nilaiPersetujuan + outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + outputDaftarAset.SF_ROW_WASDAL_PT_HIBAH[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        SvcWasdalPTHibahDetilCud.OutputParameters dOutSimpanAset;
        SvcWasdalPTHibahDetilCud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                ...
                simpanDftrAset = new SvcWasdalPTHibahDetilCud.execute_pttClient();
                simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(AppPengguna.SvcWasdalPTHibahDetilCud.OutputParameters dataOut);

        private void dsDaftarAset(AppPengguna.SvcWasdalPTHibahDetilCud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH dataPenyama;
                        ...
                        for (int i = 0; i < daftarAsetSalinan.Count; i++)
                        {
                            ...
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH daftarAsetTerpilih = (SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH)daftarTerpilih[i];
        ...
        {
            SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void gvDaftarAset_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH daftarAsetTerpilih;
                daftarAsetTerpilih = (SvcWasdalHibahSelect.WASDALSROW_WASDAL_PT_HIBAH)view.GetRow(info.RowHandle);
