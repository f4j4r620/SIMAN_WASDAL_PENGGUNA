...
{
    public partial class ucRskKspBmnForm : UserControl
    {
        ...
        public ucRskKspBmnForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        #region Ambil Data Jenis Mitra
        SvcJnsMitraSelect.dsSelect_pttClient ambilJenisMitra;
        SvcJnsMitraSelect.OutputParameters dOutJenisMitra;

        private void getJenisMitra()
        {
            jnsMitraLoaded = false;
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcJnsMitraSelect.InputParameters input = new SvcJnsMitraSelect.InputParameters();
                input.P_MINSpecified = true;
                input.P_MIN = konfigApp.currentMin;
                input.P_MAXSpecified = true;
                input.P_MAX = konfigApp.currentMaks;
                input.P_COL = "";
                input.P_SORT = "";
                input.STR_WHERE = "";
                ambilJenisMitra = new SvcJnsMitraSelect.dsSelect_pttClient();
                ambilJenisMitra.Open();
                ambilJenisMitra.Beginexecute(input, new AsyncCallback(getDataJenisMitra), null);
            }
            catch
            {
                nonAktifkanprogressBar();
        ...
        private void getDataJenisMitra(IAsyncResult result)
        {
            try
            {
                dOutJenisMitra = ambilJenisMitra.Endexecute(result);
                ambilJenisMitra.Close();
                nonAktifkanprogressBar();
                this.Invoke(new AktifkanForm(aktifkanForm), "");
                this.Invoke(new LoadDataJenisMitra(this.loadDataJenisMitra), dOutJenisMitra);
            }
            catch
            {
                nonAktifkanprogressBar();
        ...
        private delegate void LoadDataJenisMitra(SvcJnsMitraSelect.OutputParameters outputDataRef);

        private void loadDataJenisMitra(SvcJnsMitraSelect.OutputParameters outputDataRef)
        {
            int jmlData = outputDataRef.SF_ROW_R_JNS_MITRA.Count();
            if (jmlData > 0)
            {
                teJenisMitra.Properties.DataSource = outputDataRef.SF_ROW_R_JNS_MITRA;
                teJenisMitra.EditValue = outputDataRef.SF_ROW_R_JNS_MITRA[0].KD_JNS_MITRA;
                if (dataTerpilih != null)
                {
                    //teJenisMitra.Text = dataTerpilih.j
                    //teJenisMitra.EditValue = dataTerpilih.KD_PENERBIT_SK;
                }
            }
            else jnsMitraLoaded = false;
        }
        #endregion
        ...
        private string konvCharPeriode(string kdPeriode)
        {
            string period = "";
            if (kdPeriode == "H")
            { period = "Harian"; }
        ...
        public void inisialisasiForm()
        {
            ...
            if (statusForm != "C" && statusForm != "CU")
            {
                ...
                //cbPeriode.SelectedText = konvCharPeriode(dataTerpilih.PERIODE);
                //teJenisMitra.Text=dataTerpilih.
            }
