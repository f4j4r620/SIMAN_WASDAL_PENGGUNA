    ...
    public partial class ucRskSewaBmnForm : UserControl
    {
        ...
        private void dsFileWasdal(SvcFileRekamSkCru.OutputParameters outFileWasdal)
        {
            if (outFileWasdal.PO_RESULT == "Y")
            {
               ...
               // System.IO.File.WriteAllBytes("@"+Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) +outFileWasdal.PO_NM_FILE, outFileWasdal.PO_FILE_DOK);
                FrmPuViewPdf.display(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + outFileWasdal.PO_NM_FILE);
                FrmPuViewPdf.ShowDialog();
            }
            else
            {
