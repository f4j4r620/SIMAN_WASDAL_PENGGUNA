...
{
    public partial class ucRskPenjualanForm : UserControl
    {
        ...
        public SvcWasdalJualSkSelect.WASDALSROW_READ_WASDAL_PT_JUAL dataTerpilih;
        ...
        public ucRskPenjualanForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        SvcWasdalJualSelect.OutputParameters dOutDaftarAset;
        SvcWasdalJualSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                ambilDaftarAset = new SvcWasdalJualSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalJualSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalJualSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_ROW_WASDAL_PT_JUAL.Count(); 
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].IS_TB = (outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].NUMSpecified = false;
                    //outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].DARI_TGL
                    dsGridDaftarAset.Add(outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i]);
                    nilaiPersetujuan = nilaiPersetujuan + outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        SvcWasdalJualDetailCrud.OutputParameters dOutSimpanAset;
        SvcWasdalJualDetailCrud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalJualDetailCrud.InputParameters parInp = new SvcWasdalJualDetailCrud.InputParameters();
                ...
                simpanDftrAset = new SvcWasdalJualDetailCrud.execute_pttClient();
                simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(SvcWasdalJualDetailCrud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalJualDetailCrud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL dataPenyama;
                        ...
                        for (int i = 0; i < daftarAsetSalinan.Count; i++)
                        {
                            ...
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih = (SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL)daftarTerpilih[i];
        ...
        {
            SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void gvDaftarAset_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih;
                daftarAsetTerpilih = (SvcWasdalJualSelect.WASDALSROW_WASDAL_PT_JUAL)view.GetRow(info.RowHandle);
        ...
        private void sbExport_Click_1(object sender, EventArgs e)
        {
            try
            {
                string pathToSave = "";
