...
{
    public partial class ucTukarMenukarBmn : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerPindahA221.InputParameters input;
        SvcMonSatkerPindahA221.OutputParameters output;
        SvcMonSatkerPindahA221.execute_pttClient client;
        SvcMonSatkerPindahA221.WASDALSROW_MON_PINDAH_SATKER_A221 rowData;
        public ucTukarMenukarBmn()
        {
            InitializeComponent();
        }

        public ucTukarMenukarBmn(string label, string kode)
        {
            InitializeComponent();
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerPindahA221.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerPindahA221.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerPindahA221.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerPindahA221.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_PINDAH_SATKER_A221.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_PINDAH_SATKER_A221);
        ...
        private void bandedGridView1_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                var row = (SvcMonSatkerPindahA221.WASDALSROW_MON_PINDAH_SATKER_A221)e.Row;
                if (e.IsGetData)
                {
