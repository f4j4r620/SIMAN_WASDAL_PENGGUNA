...
{
    public partial class ucHibahBmn : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerPindahA231.InputParameters input;
        SvcMonSatkerPindahA231.OutputParameters output;
        SvcMonSatkerPindahA231.execute_pttClient client;
        SvcMonSatkerPindahA231.WASDALSROW_MON_PINDAH_SATKER_A231 rowData;
        public ucHibahBmn()
        {
            InitializeComponent();
        }

        public ucHibahBmn(string label, string kode)
        {
            InitializeComponent();
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerPindahA231.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerPindahA231.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerPindahA231.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerPindahA231.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_PINDAH_SATKER_A231.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_PINDAH_SATKER_A231);
        ...
        private void bandedGridView1_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                var row = (SvcMonSatkerPindahA231.WASDALSROW_MON_PINDAH_SATKER_A231)e.Row;
                if (e.IsGetData)
                {
