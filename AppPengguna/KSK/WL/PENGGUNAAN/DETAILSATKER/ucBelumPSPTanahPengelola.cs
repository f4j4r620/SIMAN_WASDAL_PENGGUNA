...
{
    public partial class ucBelumPSPTanahPengelola : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA223.InputParameters input;
        SvcMonSatkerGunaA223.OutputParameters output;
        SvcMonSatkerGunaA223.execute_pttClient client;
        SvcMonSatkerGunaA223.WASDALSROW_MON_GUNA_SATKER_A223 rowData;

        public ucBelumPSPTanahPengelola()
        {
            InitializeComponent();
        }

        public ucBelumPSPTanahPengelola(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA223.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA223.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA223.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA223.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A223.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A223);
