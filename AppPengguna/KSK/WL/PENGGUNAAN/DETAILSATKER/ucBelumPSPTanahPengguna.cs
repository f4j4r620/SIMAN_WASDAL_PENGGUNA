...
{
    public partial class ucBelumPSPTanahPengguna : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA2231.InputParameters input;
        SvcMonSatkerGunaA2231.OutputParameters output;
        SvcMonSatkerGunaA2231.execute_pttClient client;
        SvcMonSatkerGunaA2231.WASDALSROW_MON_GUNA_SATKER_A223 rowData;

        public ucBelumPSPTanahPengguna()
        {
            InitializeComponent();
        }

        public ucBelumPSPTanahPengguna(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA2231.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA2231.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA2231.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA2231.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A2231.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A2231);
