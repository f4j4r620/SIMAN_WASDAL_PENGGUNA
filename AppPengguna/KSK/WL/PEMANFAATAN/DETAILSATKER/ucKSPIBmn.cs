...
{
    public partial class ucKSPIBmn : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerManfaatA251.InputParameters input;
        SvcMonSatkerManfaatA251.OutputParameters output;
        SvcMonSatkerManfaatA251.execute_pttClient client;
        SvcMonSatkerManfaatA251.WASDALSROW_MON_MANFAAT_SATKER_A251 rowData;
        ...
        public ucKSPIBmn(string label, string kode)
        {
            InitializeComponent();
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerManfaatA251.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerManfaatA251.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerManfaatA251.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerManfaatA251.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_MANFAAT_SATKER_A251.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_MANFAAT_SATKER_A251);
        ...
        private void bandedGridView1_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                var row = (SvcMonSatkerManfaatA251.WASDALSROW_MON_MANFAAT_SATKER_A251)e.Row;
                if (e.IsGetData)
                {
                    ...
                    {
                        if (row.TGL_SETOR.ToString().Contains("11/11/00"))
                        {
