...
namespace AppPengguna.KSK.PNBP.TMBMN
{
    public partial class frmPuSk : Form
    {
        ...
        public PilihSkTmPnbp pilihSk = null;

        SvcWasdalTukarPnbpSelectSK.OutputParameters dOutDaftarSk;
        SvcWasdalTukarPnbpSelectSK.execute_pttClient ambilDaftarSk;
        SvcWasdalTukarPnbpSelectSK.WASDALSROW_READ_WASDAL_PT_TUKAR_PNBP dataTerpilih;
        ...
        public void getInitialData()
        {
            try
            {
                ...
                myThread.Start();
                SvcWasdalTukarPnbpSelectSK.InputParameters parInp = new SvcWasdalTukarPnbpSelectSK.InputParameters();
                parInp.P_COL = "";
                ...
                parInp.STR_WHERE = String.Format("KD_PELAYANAN = '{0}' AND (ID_USER = {1} OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={2})) AND ID_TL_WASDAL_PINDAHTANGAN IS NOT NULL {3}", konfigApp.kdPelayanan, konfigApp.idUser, konfigApp.idSatker, this.strCari);
                ambilDaftarSk = new SvcWasdalTukarPnbpSelectSK.execute_pttClient();
                ambilDaftarSk.Open();
        ...
        private delegate void DsSimpanData(SvcWasdalTukarPnbpSelectSK.OutputParameters dataOut);

        private void dsSimpanData(SvcWasdalTukarPnbpSelectSK.OutputParameters dataOut)
        {
            int jmlData = dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP.Count();
            ...
            {
                dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i].NUMSpecified = false;
                dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i].TGL_SK = konfigApp.setDefaultDate(dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i].TGL_SK);
                dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i].PERIODE = konfigApp.setPeriode(dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i].PERIODE);
                dsGridData.Add(dataOut.SF_READ_WASDAL_PT_TUKAR_PNBP[i]);
            }
        ...
        private void gvAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (rowTerpilih.SelectedRowsCount > 0) dataTerpilih = (SvcWasdalTukarPnbpSelectSK.WASDALSROW_READ_WASDAL_PT_TUKAR_PNBP)rowTerpilih.GetRow(e.FocusedRowHandle);
        }
