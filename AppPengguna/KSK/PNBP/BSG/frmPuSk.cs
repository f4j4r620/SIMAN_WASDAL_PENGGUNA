...
namespace AppPengguna.KSK.PNBP.BSG
{
    public partial class frmPuSk : Form
    {
        ...
        public PilihSkManfaatPnbp pilihSkBSG = null;

        SvcWasdalManfaatPnbpSelectSk.OutputParameters dOutDaftarSk;
        SvcWasdalManfaatPnbpSelectSk.execute_pttClient ambilDaftarSk;
        SvcWasdalManfaatPnbpSelectSk.WASDALSROW_READ_WASDAL_MANFAAT_PNBP dataTerpilih;
        ...
        public void getInitialData()
        {
            try
            {
                ...
                myThread.Start();
                SvcWasdalManfaatPnbpSelectSk.InputParameters parInp = new SvcWasdalManfaatPnbpSelectSk.InputParameters();
                parInp.P_COL = "";
                ...
                parInp.STR_WHERE = String.Format("ID_TL_WASDAL_MANFAAT IS NOT NULL AND KD_PELAYANAN = '{0}' AND (ID_USER = {1} OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={2})) {3}", konfigApp.kdPelayanan,konfigApp.idUser, konfigApp.idSatker, this.strCari);
                ambilDaftarSk = new SvcWasdalManfaatPnbpSelectSk.execute_pttClient();
                ambilDaftarSk.Open();
        ...
        private delegate void DsSimpanData(SvcWasdalManfaatPnbpSelectSk.OutputParameters dataOut);

        private void dsSimpanData(SvcWasdalManfaatPnbpSelectSk.OutputParameters dataOut)
        {
            int jmlData = dataOut.SF_READ_WASDAL_MANFAAT_PNBP.Count();
            ...
            {
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].NUMSpecified = false;
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].DARI_TGL = konfigApp.setDefaultDate(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].DARI_TGL);
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].SD_TGL = konfigApp.setDefaultDate(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].SD_TGL);
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].TGL_SK = konfigApp.setDefaultDate(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].TGL_SK);
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].DARI_TGL = konfigApp.setDefaultDate(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].DARI_TGL);
                dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].PERIODE = konfigApp.setPeriode(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i].PERIODE); 

                dsGridData.Add(dataOut.SF_READ_WASDAL_MANFAAT_PNBP[i]);
            }
        ...
        public SvcWasdalManfaatPnbpSelectSk.WASDALSROW_READ_WASDAL_MANFAAT_PNBP daftarSkTerpilih;

        private void bbiSimpan_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (dataTerpilih != null)
            {
                this.pilihSkBSG(dataTerpilih);
                this.Close();
        ...
        private void gvAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (rowTerpilih.SelectedRowsCount > 0) dataTerpilih = (SvcWasdalManfaatPnbpSelectSk.WASDALSROW_READ_WASDAL_MANFAAT_PNBP)rowTerpilih.GetRow(e.FocusedRowHandle);
        }
