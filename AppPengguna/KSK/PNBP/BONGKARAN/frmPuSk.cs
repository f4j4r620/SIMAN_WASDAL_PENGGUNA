...
namespace AppPengguna.KSK.PNBP.BONGKARAN
{
    public partial class frmPuSk : Form
    {
        ...
        public PilihSkBOngkaranPnbp pilihSk = null;

        SvcTLBongkaranSelect.OutputParameters dOutDaftarSk;
        SvcTLBongkaranSelect.execute_pttClient ambilDaftarSk;
        SvcTLBongkaranSelect.WASDALSROW_TL_BONGKARAN_GRID dataTerpilih;
        ...
        public void getInitialData()
        {
            try
            {
                ...
                parInp.STR_WHERE = String.Format("ID_TL_WASDAL_BNK IS NOT NULL AND KD_PELAYANAN = '{0}' AND (ID_USER = {1} OR (TIPE_PEMOHON='SATKER' AND ID_PEMOHON={2})) {3}", konfigApp.kdPelayanan, konfigApp.idUser, konfigApp.idSatker, this.strCari);
                ambilDaftarSk = new SvcTLBongkaranSelect.execute_pttClient();
                ambilDaftarSk.Open();
        ...
        private delegate void DsSimpanData(SvcTLBongkaranSelect.OutputParameters dataOut);

        private void dsSimpanData(SvcTLBongkaranSelect.OutputParameters dataOut)
        {
        ...
        private void gvAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (rowTerpilih.SelectedRowsCount > 0) dataTerpilih = (SvcTLBongkaranSelect.WASDALSROW_TL_BONGKARAN_GRID)rowTerpilih.GetRow(e.FocusedRowHandle);
        }
