...
{
    public partial class ucRTLPMPGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPMPSk.DataSource = null;
                gcGridPMPSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPMPSk.RefreshDataSource();
            }
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalPmpSkSelect.WASDALSROW_READ_WASDAL_PT_MODAL)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPMPSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPMPSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPMPSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPMPSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPMPSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPMPSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPMPSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPMPSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
