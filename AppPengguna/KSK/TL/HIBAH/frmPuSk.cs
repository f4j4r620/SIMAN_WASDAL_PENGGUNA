...
namespace AppPengguna.KSK.TL.HIBAH    
{
    public partial class frmPuSk : Form
    {
        ...
        public PilihSkHibah pilihSkHibah = null;  

        SvcWasdalHibahSkSelect.OutputParameters dOutDaftarSk;
        SvcWasdalHibahSkSelect.execute_pttClient ambilDaftarSk;
        SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH dataTerpilih;
        ...
        public void getInitialData()
        {
            try
            {
                ...
                ambilDaftarSk = new SvcWasdalHibahSkSelect.execute_pttClient();
                ambilDaftarSk.Open();
        ...
        private delegate void DsSimpanData(SvcWasdalHibahSkSelect.OutputParameters dataOut);

        private void dsSimpanData(SvcWasdalHibahSkSelect.OutputParameters dataOut)
        {
            int jmlData = dataOut.SF_READ_WASDAL_PT_HIBAH.Count();
            ...
            {
                dataOut.SF_READ_WASDAL_PT_HIBAH[i].NUMSpecified = false;
                dsGridData.Add(dataOut.SF_READ_WASDAL_PT_HIBAH[i]);
            }
        ...
        public SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH daftarSkTerpilih;

        private void bbiSimpan_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (dataTerpilih != null)
            {
                this.pilihSkHibah(dataTerpilih);
                this.Close();
        ...
        private void gvAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            //    dataTerpilih = (SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH)rowTerpilih.GetRow(e.FocusedRowHandle);
            //}
            //catch (Exception ex)
            //{
            //    dataTerpilih = null;
            //}
            if (gvSk.SelectedRowsCount > 0)
            {
                dataTerpilih = null;
                rowTerpilih = sender as GridView;
                dataTerpilih = (SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH)rowTerpilih.GetRow(e.FocusedRowHandle);
