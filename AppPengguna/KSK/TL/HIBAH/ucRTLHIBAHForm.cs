...
{
    public partial class ucRTLHIBAHForm : UserControl
    {
        public ToggleProgressBar toggleProgressBar;
        public SimpanDataRtl simpanDataRtlHIBAH;    
        ...
        public SvcWasdalPindahtanganTdLanjutSelect.WASDALSROW_TL_SELECT_WASDAL_PT dataTerpilih;
        public SvcWasdalHibahSkSelect.WASDALSROW_READ_WASDAL_PT_HIBAH skTerpilih;
        ...
        public string nmFileTL = "";

        public ucRTLHIBAHForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        public void inisialisasiForm()
        {
            ...
            if (statusForm != "C" && statusForm != "CU")
            {
                ...
                deTglBuktiLaksana.EditValue = TimeZoneInfo.ConvertTime((DateTime)dataTerpilih.TGL_BUKTI_LAKSANA, konfigApp.timeInfo);

                cbJnsDokLaksana.EditValue = dataTerpilih.JNS_BUKTI_LAKSANA;
            ...
            else
            {
                ...
                //teFileBukti.Text = skTerpilih.NM_FILE;
            }
            ...
            switch (statusForm)
            {
                ...
                case "CU":
                    ...
                    idSkTL = dataTerpilih.ID_TL_WASDAL_PINDAHTANGAN;
                    teNomorSk.Focus();
            ...
            if (cbJnsDokLaksana.SelectedIndex != -1 && teNomorSk.Text.Trim() != "" && teNoBuktiLaksana.Text != "" && teKetBukti.Text != "" && filePath != null && teFileBukti.Text.Trim() != "")
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                {
                    try
                    {
                        if (statusForm == "C")
                        {
                            idSkTL = konfigApp.getGlobalId("ID_TL_WASDAL_PINDAHTANGAN");
                        }

                        simpanDataRtlHIBAH(statusForm);
        ...
        SvcWasdalPTTdAsetSelect.OutputParameters dOutAsetSewa;
        SvcWasdalPTTdAsetSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                myThread.Start();
                svcInstansi = new SvcInstansiSelect.dsSelect_pttClient();
                SvcWasdalPTTdAsetSelect.InputParameters parInput = new SvcWasdalPTTdAsetSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                ambilDaftarAset = new SvcWasdalPTTdAsetSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalPTTdAsetSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalPTTdAsetSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_TL_DTLSELECT_WASDAL_PT.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_TL_DTLSELECT_WASDAL_PT[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_PT[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_PT[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_PT[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_PT[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        HIBAH.frmPuAset formDaftarAsetSatker;
        ...
        private void sbTambah_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                {
                    formDaftarAsetSatker = new HIBAH.frmPuAset()
                    {
                        toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
        ...
        SvcWasdalPindahtanganTdLanjutDetilCud.OutputParameters dOutSimpanAset;
        SvcWasdalPindahtanganTdLanjutDetilCud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalPindahtanganTdLanjutDetilCud.InputParameters parInp = new SvcWasdalPindahtanganTdLanjutDetilCud.InputParameters();
                ...
                parInp.P_ID_TL_WASDAL_PINDAHTANGAN = this.idSkTL;
                ...
                simpanDftrAset = new SvcWasdalPindahtanganTdLanjutDetilCud.execute_pttClient();
                //simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(SvcWasdalPindahtanganTdLanjutDetilCud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalPindahtanganTdLanjutDetilCud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT dataPenyama;
                        ...
                        {
                            SvcWasdalPTHibahSelect.WASDALSROW_WASDAL_PT_HIBAH daftarAsetTerpilih = (SvcWasdalPTHibahSelect.WASDALSROW_WASDAL_PT_HIBAH)daftarAsetSalinan[i];
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT daftarAsetTerpilih = (SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PERSETUJUAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS;
        ...
        {
            SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalPTTdAsetSelect.WASDALSROW_TL_DTLSELECT_WASDAL_PT)rowTerpilih.GetRow(e.FocusedRowHandle);
