...
{
    public partial class ucRTLKpBmnGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridKPBMNSk.DataSource = null;
                gcGridKPBMNSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridKPBMNSk.RefreshDataSource();
            }
            gvGridKPBMNSk.BestFitColumns();
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalManfaatTdLanjutSelect.WASDALSROW_TL_SELECT_WASDAL_MANFAAT)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridKPBMNSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridKPBMNSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridKPBMNSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridKPBMNSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridKPBMNSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridKPBMNSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridKPBMNSk.FocusedColumn.ToString();
                    fieldDicari = gvGridKPBMNSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
