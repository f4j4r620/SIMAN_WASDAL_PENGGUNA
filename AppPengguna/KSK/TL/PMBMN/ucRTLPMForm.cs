...
{
    public partial class ucRTLPMForm : UserControl
    {
        public ToggleProgressBar toggleProgressBar;
        public SimpanDataRtl simpanDataRtlPM;    
        ...
        public SvcWasdalHapusSkTLSelect.WASDALSROW_SK_WASDAL_HAPUS_TL skTerpilih;
        ...
        public decimal? idSkTL;
        public decimal? idSkWasdal;
        public ucRTLPMForm(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        private void resetFormRtkSwBmn()
        {
            ...
            teFileBukti.Text = "";
            cbJnsDokLaksana.EditValue = null;
            teKuantitas.Text = "";
        ...
        public int jnsBuktiLaksana()
        {
            ...
            switch (dataTerpilih.JNS_BUKTI_LAKSANAAN)
            {
                case "Keputusan Pelaksanaan":
                    cbJnsDokLaksana.SelectedIndex = 0;
                    break;
                case "Perjanjian Sewa":
                    cbJnsDokLaksana.SelectedIndex = 1;
                    break;
                case "Laporan Kepada Pengelola":
        ...
        public string jnsBuktiStr(int jns)
        {
            ...
            switch (jns)
            {
                case 0:
                    ...
                    break;
                case 1:
                    jnsBuktiStr = "Perjanjian Sewa";
                    break;
                case 2:
                    jnsBuktiStr = "Laporan Kepada Pengelola";
                    break;
                case 3:
        ...
        public void inisialisasiForm()
        {
            ...
            if (statusForm != "C" && statusForm != "CU")
            {
                ...
                idSkWasdal = dataTerpilih.ID_SK_WASDAL_HAPUS;
                idSkTL = dataTerpilih.ID_TL_WASDAL_PENGHAPUSAN;
            ...
            else
            {
                ...
                idSkWasdal = skTerpilih.ID_SK_WASDAL_HAPUS;
            ...
            if (cbJnsDokLaksana.SelectedIndex != -1 && teNomorSk.Text.Trim() != "" && teNoBuktiLaksana.Text != "" && teKetBukti.Text != "" && filePath!=null && teFileBukti.Text.Trim() != "" )
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                {
                    try
                    {
                        if (statusForm == "C")
                        {
                            idSkTL = konfigApp.getGlobalId("ID_TL_WASDAL_PENGHAPUSAN");
                        }
                        simpanDataRtlPM(statusForm);
                    }
                    catch (Exception ex)
                    {
        ...
        SvcWasdalHapusTdAsetSelect.OutputParameters dOutAsetSewa;
        SvcWasdalHapusTdAsetSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalHapusTdAsetSelect.InputParameters parInput = new SvcWasdalHapusTdAsetSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                ambilDaftarAset = new SvcWasdalHapusTdAsetSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalHapusTdAsetSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalHapusTdAsetSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_ROW_WASDAL_PENGHAPUSAN_TL.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PENGHAPUSAN_TL[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    dOutAsetSewa.SF_ROW_WASDAL_PENGHAPUSAN_TL[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_ROW_WASDAL_PENGHAPUSAN_TL[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_ROW_WASDAL_PENGHAPUSAN_TL[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_ROW_WASDAL_PENGHAPUSAN_TL[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        PMBMN.frmPuAset formDaftarAsetSatker;
        PMBMN.frmPuAset formDaftarAsetKorwil;
        PMBMN.frmPuAset formDaftarAsetEselon1;
        PMBMN.frmPuAset formDaftarAsetKl;
        ...
        private void sbTambah_Click(object sender, EventArgs e)
        {
            if (teJenisPemohon.Text == konfigApp.levelSatker)
            {
                ...
                {
                    formDaftarAsetSatker = new PMBMN.frmPuAset()
                    {
                        toggleProgressBar = new ToggleProgressBar(toggleProgBarPu),
        ...
        SvcWasdalHapusTdLanjutDetilCud.OutputParameters dOutSimpanAset;
        SvcWasdalHapusTdLanjutDetilCud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalHapusTdLanjutDetilCud.InputParameters parInp = new SvcWasdalHapusTdLanjutDetilCud.InputParameters();
               
                parInp.P_SK_KEPUTUSAN = teNomorSk.Text.Trim();
                parInp.P_ID_ASET = (_daftarIdAset=="")? null : _daftarIdAset;
                if (Convert.ToString(_modeCudAset) == "C" || Convert.ToString(_modeCudAset)=="D")
                {
                    parInp.P_NO_BUKTI_LAKSANA = teNoBuktiLaksana.Text;
                }
                parInp.P_ID_SK_WASDAL_HAPUS = idSkWasdal;
                parInp.P_ID_SK_WASDAL_HAPUSSpecified = true;
                parInp.P_ID_TL_WASDAL_PENGHAPUSAN = idSkTL;
                ...
                simpanDftrAset = new SvcWasdalHapusTdLanjutDetilCud.execute_pttClient();
                //simpanDftrAset.Open();
        ...
        private void goSimpanDftrAset(IAsyncResult result)
        {
            ...
            catch (Exception e)
            {
                ...
                MessageBox.Show(konfigApp.teksDialog+"\r\n"+dOutSimpanAset.PO_RESULT_MESSAGE, konfigApp.judulKonfirmasi);
            }
        }

        private delegate void DsDaftarAset(SvcWasdalHapusTdLanjutDetilCud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalHapusTdLanjutDetilCud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL dataPenyama;
                        ...
                        {
                            SvcWasdalHapusSelect.WASDALSROW_WASDAL_HAPUS daftarAsetTerpilih = (SvcWasdalHapusSelect.WASDALSROW_WASDAL_HAPUS)daftarAsetSalinan[i];
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL daftarAsetTerpilih = (SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PERSETUJUAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS;
        ...
        {
            SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalHapusTdAsetSelect.WASDALSROW_WASDAL_PENGHAPUSAN_TL)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void formUbahPenetapan()
        {
            if (statusForm != "A")
            {
                ...
                //formPuPenetapan.kodeStatus = dataAsetTerpilih.k;
        ...
        private void initDok(string select)
        {
            ...
            try
            {
                ...
                if (uploadSelect.Equals("CE"))
                {
                    ...
                    parInp.P_TGL_UPLOAD = DateTime.Now.ToString("dd-MM-yyyy");
                }
                parInp.P_KD_PELAYANAN = konfigApp.kdPelayanan;
        ...
        private void sbPilih_Click(object sender, EventArgs e)
        {
            ...
            if (ofdDok.ShowDialog() == DialogResult.OK && ofdDok.SafeFileName != "File PDF")
            {
