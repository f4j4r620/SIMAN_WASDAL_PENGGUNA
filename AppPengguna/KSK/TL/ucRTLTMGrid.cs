...
{
    public partial class ucRTLTMGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridTMSk.DataSource = null;
                gcGridTMSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridTMSk.RefreshDataSource();
            }
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalTmSkSelect.WASDALSROW_READ_WASDAL_PT_TUKAR)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridTMSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridTMSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridTMSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridTMSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridTMSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridTMSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridTMSk.FocusedColumn.ToString();
                    fieldDicari = gvGridTMSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
