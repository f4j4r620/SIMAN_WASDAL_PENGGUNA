...
{
    public partial class ucRTLBGSForm1 : UserControl
    {
        public ToggleProgressBar toggleProgressBar;
        public SimpanDataRtl simpanDataRtlBGS;
        ...
        public SvcWasdalManfaatTdLanjutSelect.WASDALSROW_TL_SELECT_WASDAL_MANFAAT dataTerpilih;
        public SvcWasdalManfaatSkSelect.WASDALSROW_READ_WASDAL_MANFAAT skTerpilih;
        ...
        public ucRTLBGSForm1(string _status)
        {
            InitializeComponent();
            statusForm = _status;
        ...
        public void inisialisasiForm()
        {
            ...
            if (statusForm != "C" && statusForm != "CU")
            {
                ...
                idSkTL = dataTerpilih.ID_TL_WASDAL_MANFAAT;
            ...
            switch (statusForm)
            {
                ...
                case "U":
                    ...
                    this.idSkTL = dataTerpilih.ID_TL_WASDAL_MANFAAT;
            ...
            if (cbJnsDokLaksana.SelectedIndex != -1 && teNomorSk.Text.Trim() != "" && teNoBuktiLaksana.Text != "" && teKetBukti.Text != "" && filePath != null && teFileBukti.Text.Trim() != "")
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                {
                    try
                    {
                        if (statusForm == "C")
                        {
                            idSkTL = konfigApp.getGlobalId("ID_TL_WASDAL_MANFAAT");
                        }

                        simpanDataRtlBGS(statusForm);
                    }
                    catch (Exception ex)
                    {
        ...
        SvcWasdalManfaatTdAsetSelect.OutputParameters dOutAsetSewa;
        SvcWasdalManfaatTdAsetSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                myThread.Start();
                svcInstansi = new SvcInstansiSelect.dsSelect_pttClient();
                SvcWasdalManfaatTdAsetSelect.InputParameters parInput = new SvcWasdalManfaatTdAsetSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                ambilDaftarAset = new SvcWasdalManfaatTdAsetSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalManfaatTdAsetSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalManfaatTdAsetSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_TL_DTLSELECT_WASDAL_MANFAAT.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_TL_DTLSELECT_WASDAL_MANFAAT[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    //dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i].IS_TB = (dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_TL_DTLSELECT_WASDAL_MANFAAT[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        SvcWasdalManfaatTdLanjutDetilCud.OutputParameters dOutSimpanAset;
        SvcWasdalManfaatTdLanjutDetilCud.execute_pttClient simpanDftrAset;
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalManfaatTdLanjutDetilCud.InputParameters parInp = new SvcWasdalManfaatTdLanjutDetilCud.InputParameters();
                ...
                parInp.P_ID_TL_WASDAL_MANFAAT = this.idSkTL;
                ...
                simpanDftrAset = new SvcWasdalManfaatTdLanjutDetilCud.execute_pttClient();
                //simpanDftrAset.Open();
        ...
        private delegate void DsDaftarAset(SvcWasdalManfaatTdLanjutDetilCud.OutputParameters dataOut);

        private void dsDaftarAset(SvcWasdalManfaatTdLanjutDetilCud.OutputParameters dataOut)
        {
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT dataPenyama;
                        ...
                        {
                            SvcWasdalManfaatSelect.WASDALSROW_WASDAL_MANFAAT daftarAsetTerpilih = (SvcWasdalManfaatSelect.WASDALSROW_WASDAL_MANFAAT)daftarAsetSalinan[i];
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT daftarAsetTerpilih = (SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PERSETUJUAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS;
        ...
        {
            SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalManfaatTdAsetSelect.WASDALSROW_TL_DTLSEL_WASDAL_MANFAAT)rowTerpilih.GetRow(e.FocusedRowHandle);
