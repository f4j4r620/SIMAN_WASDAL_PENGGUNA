...
namespace AppPengguna.KSK.TL.PSPBMN
{
    ...
    public partial class frmTL : DevExpress.XtraEditors.XtraForm
    {
        ...
        SvcWasdalPSPAllCud.call_pttClient svcWasdalPSPAllCud = null;
        SvcWasdalPSPAllCud.OutputParameters dataoutWlPSPBMNDetailCrud = null;


        ucRTLPspBmnForm ucPSPBMN;
        ...
        public frmTL(ucRTLPspBmnForm _ucPSPBMN, string _daftarAset)
        {
            InitializeComponent(); Icon = Properties.Resources.logo_2016;
            this.ucPSPBMN = _ucPSPBMN;
        ...
        private void getStatus()
        {
            try
            {
                ...
                input.STR_WHERE = " ((KD_STATUS = '01') OR (KD_STATUS = '02') OR (KD_STATUS = '03') OR (KD_STATUS = '06') OR (KD_STATUS = '99'))";
        ...
        public void crudWlDetailPSPBMN()
        {
            try
            {
                ...
                SvcWasdalPSPAllCud.InputParameters parInp = new SvcWasdalPSPAllCud.InputParameters();
                ...
                svcWasdalPSPAllCud = new SvcWasdalPSPAllCud.call_pttClient();
                if (this.skKeputusan != "-")
                {
                    svcWasdalPSPAllCud.Beginexecute(parInp, new AsyncCallback(resultPSPBMNDetailCrud), "");
                }
                else
                {
                    this.setThread(false);
        ...
        private void resultPSPBMNDetailCrud(IAsyncResult result)
        {
            try
            {
                dataoutWlPSPBMNDetailCrud = svcWasdalPSPAllCud.Endexecute(result);
                svcWasdalPSPAllCud.Close();
                this.setThread(false);
                this.Invoke(new ResponWlPSPBMNDetailCrud(this.responWlPSPBMNDetailCrud), dataoutWlPSPBMNDetailCrud);
        ...
        private delegate void ResponWlPSPBMNDetailCrud(SvcWasdalPSPAllCud.OutputParameters dataoutWlPSPBMNDetailCrud);
        private void responWlPSPBMNDetailCrud(SvcWasdalPSPAllCud.OutputParameters dataoutWlPSPBMNDetailCrud)
        {
            if (dataoutWlPSPBMNDetailCrud.PO_RESULT == "Y")
            {

                //ucPSPBMN.tabSelect();
                
                this.Close();
                FrmKoorSatker frm = new FrmKoorSatker();
                //frm.initGridRkmTlPSPSBMN();
                ucPSPBMN.getDaftarAset();
                
            }
            else
            {
                MessageBox.Show(dataoutWlPSPBMNDetailCrud.PO_RESULT_MESSAGE, konfigApp.judulGagal);
            }
        }
        ...
        private void sbSave_Click(object sender, EventArgs e)
        {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    parInp.P_NM_PHK_LAIN = txtNmPhkLain.Text;
