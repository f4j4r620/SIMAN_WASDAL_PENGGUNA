...
namespace AppPengguna.KSK.TL
{
    public partial class ucRTLPBKSLGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPBKSLSk.DataSource = null;
                gcGridPBKSLSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPBKSLSk.RefreshDataSource();
            }
        }
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPBKSLSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPBKSLSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPBKSLSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPBKSLSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPBKSLSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPBKSLSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPBKSLSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPBKSLSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
