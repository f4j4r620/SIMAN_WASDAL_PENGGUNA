...
{
    public partial class ucRTLPMGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPMSk.DataSource = null;
                gcGridPMSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPMSk.RefreshDataSource();
            }
        }
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPMSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPMSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPMSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPMSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPMSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPMSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPMSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPMSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
