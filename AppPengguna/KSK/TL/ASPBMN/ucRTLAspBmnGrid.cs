...
{
    public partial class ucRTLAspBmnGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridASPBMNSk.DataSource = null;
                gcGridASPBMNSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridASPBMNSk.RefreshDataSource();
            }
            gvGridASPBMNSk.BestFitColumns();
        }
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridASPBMNSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridASPBMNSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridASPBMNSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridASPBMNSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridASPBMNSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridASPBMNSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridASPBMNSk.FocusedColumn.ToString();
                    fieldDicari = gvGridASPBMNSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
