...
{
    public partial class ucRTLPBKPPGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPBKPPSk.DataSource = null;
                gcGridPBKPPSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPBKPPSk.RefreshDataSource();
            }
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalHapusSkSelect.WASDALSROW_READ_WASDAL_HAPUS)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPBKPPSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPBKPPSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPBKPPSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPBKPPSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPBKPPSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPBKPPSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPBKPPSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPBKPPSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
