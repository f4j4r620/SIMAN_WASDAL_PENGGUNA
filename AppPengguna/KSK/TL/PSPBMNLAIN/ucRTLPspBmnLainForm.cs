...
namespace AppPengguna.KSK.TL.PSPBMNLAIN
{
    public partial class ucRTLPspBmnLainForm : UserControl
    {
        ...
        private void resetFormRtkSwBmn()
        {
            ...
            txtNmPhkLain.Text = "";
            txtAlamatPhkLain.Text = "";

            teJangkaWaktu.Text = "";
            cbPeriode.Text = "";
            teDariTgl.EditValue = null;
            teSampaiTgl.EditValue = null;
        ...
        public void inisialisasiForm()
        {
            ...
            if (statusForm != "C" && statusForm != "CU")
            {
                ...
                idSkTL = dataTerpilih.ID_TL_WASDAL_GUNA;
                ...
                txtNmPhkLain.Text = dataTerpilih.UR_PHK_LAIN;
                txtAlamatPhkLain.Text = dataTerpilih.ALAMAT_PHK_LAIN;
                teFileBukti.Text = "-";

                teJangkaWaktu.Text = dataTerpilih.JANGKA_WAKTU.ToString();
                ...
                teNoPerjanjian.Text = dataTerpilih.NO_REKAM_PERJANJIAN;
                teTanggalPerjanjian.EditValue = Convert.ToDateTime(dataTerpilih.TGL_REKAM_PERJANJIAN);
                teStatusPerjanjian.SelectedIndex = (dataTerpilih.STATUS_PERJANJIAN == "DIBERHENTIKAN") ? 1 : 0;

                if (teStatusPerjanjian.Text == "DIBERHENTIKAN")
                {
                    teNoBAST.Text = dataTerpilih.NO_BUKTI_LAKSANA;
                    teTanggalBAST.EditValue = Convert.ToDateTime(dataTerpilih.TGL_BUKTI_LAKSANA);
                    teFileBAST.Text = dataTerpilih.NM_FILE_BAST;
                }
            ...
            else
            {
                ...
                txtNmPhkLain.Text = skTerpilih.UR_PHK_LAIN;
                txtAlamatPhkLain.Text = skTerpilih.ALAMAT_PHK_LAIN;
                //getDaftarAset();
            }
            ...
            txtNmPhkLain.Properties.ReadOnly = true;
            txtAlamatPhkLain.Properties.ReadOnly = true;
            
            switch (statusForm)
            {
                case "C":
                    idSkWasdal = skTerpilih.ID_SK_WASDAL;
                    break;
                case "CU":
                ...
                case "U":
                    ...
                    this.idSkTL = dataTerpilih.ID_TL_WASDAL_GUNA;
                    ...
                    txtNmPhkLain.Enabled = false;
                    txtAlamatPhkLain.Enabled = false;
                    break;
                case "A":
                    ...
                    teFileBukti.Properties.ReadOnly = true;
                    btnUploadFileBukti.Enabled = false;
                    txtNmPhkLain.Enabled = false;
                    txtAlamatPhkLain.Enabled = false;
                    teNoPerjanjian.Properties.ReadOnly = true;
                    teTanggalPerjanjian.Properties.ReadOnly = true;
                    teStatusPerjanjian.Properties.ReadOnly = true;
                    teNoBAST.Properties.ReadOnly = true;
                    teTanggalBAST.Properties.ReadOnly = true;
                    teFileBAST.Properties.ReadOnly = true;
                    BtnPilihFileBAST.Enabled = false;
                    break;
            }
        ...
        private void TanggalOtomatis(DateTime? date = null)
        {
            if (teJangkaWaktu.EditValue != "")
            {
                string _per = cbPeriode.Text;
                DateTime setTgl = DateTime.Now;
                //DateTime sekarang = (DateTime)teDariTgl.EditValue;
                DateTime sekarang = (teDariTgl.EditValue == null) ? DateTime.Now : (DateTime)teDariTgl.EditValue;
            ...
            if (cbJnsDokLaksana.SelectedIndex != -1 && teNomorSk.Text.Trim() != "" && teNoBuktiLaksana.Text != "" && teKetBukti.Text != "" && filePath != null && teFileBukti.Text.Trim() != "")
            {
                if (statusForm == "C" || statusForm == "U" || statusForm == "CU")
                {
                    try
                    {
                        if (statusForm == "C")
                        {
                            idSkTL = konfigApp.getGlobalId("ID_TL_WASDAL_GUNA");
                        }
        ...
        SvcWasdalPspLainTdAsetSelect.OutputParameters dOutAsetSewa;
        SvcWasdalPspLainTdAsetSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2 dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                myThread.Start();
                svcInstansi = new SvcInstansiSelect.dsSelect_pttClient();
                SvcWasdalPspLainTdAsetSelect.InputParameters parInput = new SvcWasdalPspLainTdAsetSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                parInput.P_SORT = "";
                parInput.STR_WHERE = String.Format(" (SK_KEPUTUSAN = '{0}' AND NO_BUKTI_LAKSANA = '{1}' ) ", teNomorSk.Text.Trim(),teNoBuktiLaksana.Text);
                ambilDaftarAset = new SvcWasdalPspLainTdAsetSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalPspLainTdAsetSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalPspLainTdAsetSelect.OutputParameters outputDaftarAset)
        {
            int jmlData = outputDaftarAset.SF_ROW_WASDAL_PENGGUNAAN_TL2.Count();
            decimal? nilaiPersetujuan = 0;
            decimal? jmlKuantitas = 0;
            dsGridDaftarAset = new ArrayList();
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PENGGUNAAN_TL2[0].KD_BRG != "-")
            {
                for (int i = 0; i < jmlData; i++)
                {
                    dOutAsetSewa.SF_ROW_WASDAL_PENGGUNAAN_TL2[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_ROW_WASDAL_PENGGUNAAN_TL2[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_ROW_WASDAL_PENGGUNAAN_TL2[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_ROW_WASDAL_PENGGUNAAN_TL2[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
        ...
        private void simpanDaftarAset(string _daftarIdAset, char _modeCudAset)
        {
            try
            {
                ...
                parInp.P_ID_SK_WASDAL = this.idSkWasdal;
                parInp.P_ID_SK_WASDALSpecified = true;
                parInp.P_ID_TL_WASDAL_GUNA = this.idSkTL;
        ...
        private void goSimpanDftrAset(IAsyncResult result)
        {
            ...
            catch (Exception e)
            {
                ...
                MessageBox.Show(konfigApp.teksDialog+" "+dOutSimpanAset.PO_RESULT_MESSAGE, konfigApp.judulKonfirmasi);
            }
        }
            ...
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2 dataPenyama;
                        ...
                        for (int i = 0; i < daftarAsetSalinan.Count; i++)
                        {
                            ...
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2 daftarAsetTerpilih = (SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PERSETUJUAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS;
        ...
        {
            SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2 daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalPspLainTdAsetSelect.WASDALSROW_WASDAL_PENGGUNAAN_TL2)rowTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private void btnUploadFileBukti_Click(object sender, EventArgs e)
        {
            ...
            if (ofdDok.ShowDialog() == DialogResult.OK && ofdDok.FileName != "File PDF")
            {
                ...
                namaFileUpload = ofdDok.SafeFileName;
        ...
        SvcWasdalManfaatTdLanjutFile2.execute_pttClient getFileWasdal;
        SvcWasdalManfaatTdLanjutFile2.OutputParameters outFileWasdal;
        private void btnViewDok_Click(object sender, EventArgs e)
        {
            //_statusCrud = "Download";
            //getInitFileUpload();
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalManfaatTdLanjutFile2.InputParameters parInp = new SvcWasdalManfaatTdLanjutFile2.InputParameters();
                parInp.P_NO_BUKTI_LAKSANA = teNoBuktiLaksana.Text;
                parInp.P_SK_KEPUTUSAN = teNomorSk.Text;
                parInp.P_KD_PELAYANAN = "03";
                parInp.P_ID_TL_WASDAL = idSkTL;
                parInp.P_ID_TL_WASDALSpecified = true;
                parInp.P_ID_SK_WASDAL = idSkWasdal;
                parInp.P_ID_SK_WASDALSpecified = true;
                parInp.P_SELECT = "R";
                getFileWasdal = new SvcWasdalManfaatTdLanjutFile2.execute_pttClient();
        ...
        private delegate void DsFileWasdal(SvcWasdalManfaatTdLanjutFile2.OutputParameters outFileWasdal);
        private void dsFileWasdal(SvcWasdalManfaatTdLanjutFile2.OutputParameters outFileWasdal)
        {
            if(outFileWasdal.PO_RESULT=="Y")
            {
                PU.FrmPuViewPdf FrmPuViewPdf = new PU.FrmPuViewPdf();

                System.IO.File.WriteAllBytes("DOK_BUKTI_" + teNoBuktiLaksana.Text.Replace('/', '-') + ".pdf", outFileWasdal.PO_FILE_BUKTI);
                FrmPuViewPdf.display("DOK_BUKTI_" + teNoBuktiLaksana.Text.Replace('/', '-') + ".pdf");
        ...
        private void teJangkaWaktu_EditValueChanged_1(object sender, EventArgs e)
        {
            this.TanggalOtomatis();
        }

        private void cbPeriode_EditValueChanged_1(object sender, EventArgs e)
        {
            this.TanggalOtomatis();
        }
        ...
        private SvcFileRekamTL.execute_pttClient fetchingFile;
        private SvcFileRekamTL.OutputParameters dataOutFile;
        ...
        private void getInitFileUpload()
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcFileRekamTL.InputParameters parInp = new SvcFileRekamTL.InputParameters();
                ...
                if (_statusCrud == "Upload")
                {
                    ...
                    parInp.P_ID_SK_WASDAL = this.idSkWasdal;
                    parInp.P_ID_SK_WASDALSpecified = true;
                    parInp.P_ID_TL_WASDAL = this.idSkTL;
                    parInp.P_ID_TL_WASDALSpecified = true;
                    parInp.P_NO_BUKTI_LAKSANA = teNoBuktiLaksana.Text;

                    parInp.P_FILE_DOK = konfigApp.FileToByteArray(teFileBukti.Text);
                    parInp.P_SK_KEPUTUSAN = teNomorSk.Text;
                    parInp.P_KD_PELAYANAN = "03";
                ...
                fetchingFile = new SvcFileRekamTL.execute_pttClient();
        ...
        private delegate void dsFileUploadRead(SvcFileRekamTL.OutputParameters dataout);

        private void dsfileUploadRead(SvcFileRekamTL.OutputParameters dataout)
        {
            if (dataout.PO_RESULT == "Y")
            {
                if (_statusCrud == "Upload")
                ...
                else
                {
                    ...
                    System.IO.File.WriteAllBytes(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + dataOutFile.PO_NM_FILE + ".pdf", dataout.PO_FILE_BUKTI);
        ...
        private void sbUpload_Click(object sender, EventArgs e)
        {
            ...
            if (teFileBukti.Text.Trim() != null && teFileBukti.Text.Trim() != "")
            {
                _statusCrud = "Upload";
                getInitFileUpload();
            }
        ...
        private void teStatusPerjanjian_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (teStatusPerjanjian.Text == "DIBERHENTIKAN")
                {
                    GroupBAST.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
                }
                else
                {
                    GroupBAST.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
                }
            }
            catch (Exception)
            {
        ...
        public string namaFileUploadBAST = "";
        private void BtnPilihFileBAST_Click(object sender, EventArgs e)
        {
            ofdDok.Title = "Pilih File";
            ofdDok.FileName = "File PDF";
            ofdDok.Filter = "(File PDF)|*.pdf";
            ...
            if (ofdDok.ShowDialog() == DialogResult.OK && ofdDok.FileName != "File PDF")
            {
                teFileBAST.Text = ofdDok.SafeFileName ;
                namaFileUploadBAST = ofdDok.InitialDirectory + ofdDok.FileName;
            }
        }
        ...
        SvcWasdalLainTLBASTFile.execute_pttClient client;
        SvcWasdalLainTLBASTFile.OutputParameters output;
        private void BtnViewFileBST_Click(object sender, EventArgs e)
        {
            try
            {
                myThread = new Thread(new ThreadStart(aktifkanProgresBar));
                myThread.Start();
                SvcWasdalLainTLBASTFile.InputParameters parInp = new SvcWasdalLainTLBASTFile.InputParameters();
                parInp.P_ID_SK_WASDAL = dataTerpilih.ID_SK_WASDAL;
                parInp.P_ID_SK_WASDALSpecified = true;
                parInp.P_ID_TL_WASDAL_GUNA = dataTerpilih.ID_TL_WASDAL_GUNA;
                parInp.P_ID_TL_WASDAL_GUNASpecified = true;
                parInp.P_SELECT = "R";
                client = new SvcWasdalLainTLBASTFile.execute_pttClient();
                client.Open();
                client.Beginexecute(parInp, new AsyncCallback(this.getDataFileBAST), null);
            }
            catch (Exception)
            {
                nonAktifkanprogressBar();
        ...
        public void getDataFileBAST(IAsyncResult result)
        {
            try
            {
                ...
                nonAktifkanprogressBar();
                this.Invoke(new AktifkanForm(aktifkanForm), "");
                this.Invoke(new DsFileWasdalBAST(dsFileWasdalBAST), output);
            }
            catch (Exception)
            {
                nonAktifkanprogressBar();
        ...
        public delegate void DsFileWasdalBAST(SvcWasdalLainTLBASTFile.OutputParameters output);

        public void dsFileWasdalBAST(SvcWasdalLainTLBASTFile.OutputParameters output)
        {
            try
            {
                ...
                {
                    PU.FrmPuViewPdf FrmPuViewPdf = new PU.FrmPuViewPdf();
                    System.IO.File.WriteAllBytes("DOK_BUKTI_" + output.PO_NM_FILE.Replace('/', '-') + ".pdf", output.PO_FILE_BAST);
                    FrmPuViewPdf.display("DOK_BUKTI_" + output.PO_NM_FILE.Replace('/', '-') + ".pdf");
                    FrmPuViewPdf.ShowDialog();
                }
                else{
                    MessageBox.Show(output.PO_RESULT_MESSAGE);
                }
            }
            catch (Exception)
            {
        ...
        private void cbJnsDokLaksana_SelectedValueChanged(object sender, EventArgs e)
        {
            DevExpress.XtraEditors.ComboBoxEdit cmb = (DevExpress.XtraEditors.ComboBoxEdit)sender;
            bool textReadonly = false;
            if (cmb.SelectedText.ToString() == "Laporan Berakhirnya" || cmb.SelectedText.ToString() == "BAST Pengembalian BMN") textReadonly = true;
            else textReadonly = false;

            teJangkaWaktu.Properties.ReadOnly = textReadonly;
            cbPeriode.Properties.ReadOnly = textReadonly;
        }

        private void teDariTgl_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                DevExpress.XtraEditors.DateEdit de = (DevExpress.XtraEditors.DateEdit)sender;
                TanggalOtomatis((DateTime?)de.EditValue);
            }
            catch 
            {
