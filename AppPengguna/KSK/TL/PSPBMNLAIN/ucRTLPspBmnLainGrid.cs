...
{
    public partial class ucRTLPspBmnLainGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPSPBMNLAINSk.DataSource = null;
                gcGridPSPBMNLAINSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPSPBMNLAINSk.RefreshDataSource();
            }
            gvGridPSPBMNLAINSk.BestFitColumns();
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalLainTindakLanjutSelect.WASDALSROW_READ_WASDAL_PSP_LAIN_TL2)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPSPBMNLAINSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPSPBMNLAINSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPSPBMNLAINSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPSPBMNLAINSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPSPBMNLAINSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPSPBMNLAINSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPSPBMNLAINSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPSPBMNLAINSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
