...
{
    public partial class ucPilihAsetBgs : UserControl
    {
        public ucPilihAsetBgs()
        {
            InitializeComponent();
        }
        ...
        SvcKelPilihAsetBgs.execute_pttClient fetchdata;
        SvcKelPilihAsetBgs.OutputParameters output;
        ...
        public void load()
        {
            try
            {
                ...
                myThread.Start();
                SvcKelPilihAsetBgs.InputParameters parInp = new SvcKelPilihAsetBgs.InputParameters();
                ...
                fetchdata = new SvcKelPilihAsetBgs.execute_pttClient();
        ...
        private delegate void DsRskPspBmn(SvcKelPilihAsetBgs.OutputParameters dataOut);

        private void dsRskPspBmn(SvcKelPilihAsetBgs.OutputParameters dataOut)
        {
            int jmlData = dataOut.SF_ROW_ASET_BSG.Count();
            if (dataInisial) arrayList = new ArrayList();

            foreach (SvcKelPilihAsetBgs.DBKELOLASROW_ASET_BSG item in dataOut.SF_ROW_ASET_BSG)
                arrayList.Add(item);
