...
{
    public partial class ucRTLBGSGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridBGSSk.DataSource = null;
                gcGridBGSSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridBGSSk.RefreshDataSource();
            }
        }
        ...
        private void gvGridSk_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            if (viewTerpilih.SelectedRowsCount > 0)
                dataTerpilih = (SvcWasdalManfaatSkSelect.WASDALSROW_READ_WASDAL_MANFAAT)viewTerpilih.GetRow(e.FocusedRowHandle);
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridBGSSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridBGSSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridBGSSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridBGSSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridBGSSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridBGSSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridBGSSk.FocusedColumn.ToString();
                    fieldDicari = gvGridBGSSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
