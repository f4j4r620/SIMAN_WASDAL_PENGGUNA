...
using AppPengguna.KES1.TL.PBKPP;
...
namespace AppPengguna.KES1.TL.PBKPP
{
    public partial class ucRTLPBKPPForm : UserControl
    {
        ...
        SvcWasdalManfaatSelect.OutputParameters dOutAsetSewa;
        ...
        private void loadDataDaftarAset(SvcWasdalManfaatSelect.OutputParameters outputDaftarAset)
        {
            ...
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_MANFAAT[0].KD_BRG != "-")
            {
                ...
                {
                    dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i].IS_TB = (dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_ROW_WASDAL_MANFAAT[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
