...
namespace AppPengguna.KES1.TL.PENJUALAN  
{
    public partial class ucRTLJUALForm : UserControl
    {
        ...
        SvcWasdalPTJualSelect.OutputParameters dOutAsetSewa;
        SvcWasdalPTJualSelect.execute_pttClient ambilDaftarAset;
        SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL dataAsetTerpilih;
        ...
        private void getDaftarAset()
        {
            try
            {
                ...
                myThread.Start();
                svcInstansi = new SvcInstansiSelect.dsSelect_pttClient();
                SvcWasdalPTJualSelect.InputParameters parInput = new SvcWasdalPTJualSelect.InputParameters();
                parInput.P_COL = "";
                parInput.P_COUNT = "N";
                parInput.P_MAX = 0;
                ...
                ambilDaftarAset = new SvcWasdalPTJualSelect.execute_pttClient();
        ...
        private delegate void LoadDataDaftarAset(SvcWasdalPTJualSelect.OutputParameters outputDaftarAset);

        private void loadDataDaftarAset(SvcWasdalPTJualSelect.OutputParameters outputDaftarAset)
        {
            ...
            if (jmlData >= 1 && outputDaftarAset.SF_ROW_WASDAL_PT_JUAL[0].KD_BRG != "-")
            {
                ...
                {
                    dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i].IS_TB = (dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i].IS_TB == "Y" ? konfigApp.namaTb : konfigApp.namaNonTb);
                    dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i].NUMSpecified = false;
                    dsGridDaftarAset.Add(dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i]);
                    nilaiPersetujuan = nilaiPersetujuan + dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i].NILAI_PERSETUJUAN;
                    jmlKuantitas = jmlKuantitas + dOutAsetSewa.SF_ROW_WASDAL_PT_JUAL[i].KUANTITAS;
                }
                gcDaftarAset.DataSource = null;
                gcDaftarAset.DataSource = dsGridDaftarAset;
            }
            ...
            if (dataOut.PO_RESULT == "Y")
            {
                ...
                switch (modeCudAset)
                {
                    case 'C':
                        ...
                        SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL dataPenyama;
                        ...
                        {
                            SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih = (SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL)daftarAsetSalinan[i];
                            dataPenyama = null;
                            dataPenyama = new SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL();
                    ...
                    case 'D':
                        if (daftarTerpilih != null)
                        {
                            ...
                            {
                                SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih = (SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL)daftarTerpilih[i];
                                _krgNilTetap = _krgNilTetap - daftarAsetTerpilih.NILAI_PENETAPAN;
                                _krgKuantitas = _krgKuantitas - daftarAsetTerpilih.KUANTITAS_SK;
        ...
        {
            SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL daftarAsetTerpilih;
            if (rowTerpilih != null)
            {
                ...
                else
                {
                    for (int i = 0; i < daftarTerpilih.Count; i++)
                    {
                        daftarAsetTerpilih = (SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL)daftarTerpilih[i];
        ...
        private void gvDaftarAset_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                dataAsetTerpilih = (SvcWasdalPTJualSelect.WASDALSROW_WASDAL_PT_JUAL)rowTerpilih.GetRow(e.FocusedRowHandle);
