...
{
    public partial class ucRTLPpBmnGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridPPBMNSk.DataSource = null;
                gcGridPPBMNSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridPPBMNSk.RefreshDataSource();
            }
        }
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridPPBMNSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridPPBMNSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridPPBMNSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridPPBMNSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridPPBMNSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridPPBMNSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridPPBMNSk.FocusedColumn.ToString();
                    fieldDicari = gvGridPPBMNSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
