...
{
    public partial class ucRTLBSGGrid : UserControl
    {
        ...
        public void displayData()
        {
            if (dataInisial == true)
            {
                gcGridBSGSk.DataSource = null;
                gcGridBSGSk.DataSource = dsDataSource;
            }
            else
            {
                gcGridBSGSk.RefreshDataSource();
            }
        }
        ...
        private string getFieldKolom(string judulKolom)
        {
            ...
            if (judulKolom != "" && judulKolom.Trim() != "System.Collections.ArrayList")
                kembalian = gvGridBSGSk.Columns.ColumnByName(judulKolom).FieldName;
            return kembalian;
        }
        ...
        private void jumlahKolom()
        {
            ...
            int jmlKolom = gvGridBSGSk.Columns.Count;
            int indeksBaris = 0;
            for (int i = 0; i < jmlKolom; i++)
            {
                if (gvGridBSGSk.Columns[i].FieldName != "NUM")
                {
                    teNamaKolom.Properties.Items.Insert(indeksBaris, gvGridBSGSk.Columns[i].Caption);
                    indeksBaris++;
                }
            }
            teNamaKolom.Text = "";
        ...
        {
            if (gvGridBSGSk.FocusedColumn.FieldName != "NUM")
            {
                teCari.Text = gvGridBSGSk.GetFocusedDisplayText();
                if (teCari.Text.Trim() != "")
                {
                    teNamaKolom.Text = gvGridBSGSk.FocusedColumn.ToString();
                    fieldDicari = gvGridBSGSk.FocusedColumn.FieldName;
                }
                else
                {
                    teNamaKolom.Text = "";
