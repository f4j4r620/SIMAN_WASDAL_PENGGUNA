...
{
    public partial class ucBelumPSPSTBNonDokPengguna : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA2211.InputParameters input;
        SvcMonSatkerGunaA2211.OutputParameters output;
        SvcMonSatkerGunaA2211.execute_pttClient client;
        SvcMonSatkerGunaA2211.WASDALSROW_MON_GUNA_SATKER_A221 rowData;

        public ucBelumPSPSTBNonDokPengguna()
        {
            InitializeComponent();
        }

        public ucBelumPSPSTBNonDokPengguna(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA2211.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA2211.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA2211.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA2211.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A2211.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A2211);
