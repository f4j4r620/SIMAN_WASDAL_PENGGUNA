...
{
    public partial class ucBelumPSPSTBDokPengguna : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA2221.InputParameters input;
        SvcMonSatkerGunaA2221.OutputParameters output;
        SvcMonSatkerGunaA2221.execute_pttClient client;
        SvcMonSatkerGunaA2221.WASDALSROW_MON_GUNA_SATKER_A222 rowData;

        public ucBelumPSPSTBDokPengguna()
        {
            InitializeComponent();
        }

        public ucBelumPSPSTBDokPengguna(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA2221.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA2221.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA2221.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA2221.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A2221.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A2221);
