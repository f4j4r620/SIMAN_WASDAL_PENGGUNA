...
{
    public partial class ucBelumPSPSTBDokPengelola : ucMain
    {
        ...
        ///  Service
        /// </summary>
        SvcMonSatkerGunaA222.InputParameters input;
        SvcMonSatkerGunaA222.OutputParameters output;
        SvcMonSatkerGunaA222.execute_pttClient client;
        SvcMonSatkerGunaA222.WASDALSROW_MON_GUNA_SATKER_A222 rowData;

        public ucBelumPSPSTBDokPengelola()
        {
            InitializeComponent();
        }

        public ucBelumPSPSTBDokPengelola(string label, string kode)
        {
            InitializeComponent();
            labelControl1.Text += "\nJENIS BMN : "+label;
            this.kode = kode;
        ...
        private void GetData(string where)
        {
            ...
            {
                FormNonAktif();
                input = new SvcMonSatkerGunaA222.InputParameters();
                if (dataInisial)
                {
                ...
                client = new SvcMonSatkerGunaA222.execute_pttClient();
                client.Open();
        ...
        private delegate void ShowData(SvcMonSatkerGunaA222.OutputParameters output);

        private void ShowDataGrid(SvcMonSatkerGunaA222.OutputParameters output)
        {
            try
            {
                int jmlData = output.SF_MON_GUNA_SATKER_A222.Count();
                dataCount += jmlData;
                ...
                data.AddRange(output.SF_MON_GUNA_SATKER_A222);
